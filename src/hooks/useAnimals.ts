...
import { getRandomName, getUniqueAngusName, getUniqueBoerName, getUniqueCavaloName, getUniqueFrangoName, getUniqueOxName, getUniquePorcoName } from '../names';
...
export type InventoryState = {
  ...
  sheep_milk: number;
  queijo_pecorino: number;
  iogurte_ovelha: number;
  ricota_ovelha: number;
  doce_leite_ovelha: number;
};
...
export function useAnimals({
  ...
  const getAnimalFeedType = (type: AnimalType): { feedType: 'racaoBovina' | 'racaoOvinos' | 'racaoAves' | 'racaoAquatica' | 'racaoCoelho' | 'racaoCarnivora' | 'racaoSuina' | 'racaoPeixe' | 'racaoEquina'; feedLabel: string } => {
    ...
    if (type === 'ovelha' || type === 'ovelha_leiteira' || type === 'cabra' || type === 'cabra_angora' || type === 'cabra_boer' || type === 'lhama' || type === 'alpaca') return { feedType: 'racaoOvinos', feedLabel: 'Ração de Ovinos' };
    ...
    if (type === 'tanque_tilapia' || type === 'tanque_pirarucu') return { feedType: 'racaoPeixe' as any, feedLabel: 'Ração de Peixes' };
  ...
  const getAnimalPurchasePrice = (type: AnimalType, specOverride?: FarmSpecialization): number => {
    ...
    if (type === 'ovelha_leiteira') basePrice = 180;
    if (type === 'peru') basePrice = 180;
    if (type === 'cabra_angora') basePrice = 280;
    ...
    if (type === 'cabra_boer') basePrice = 380;
    if (type === 'tanque_pirarucu') basePrice = 1000;
    ...
    const purchasePenalty =
      ...
      (spec === 'avicultura' && !['galinha','pato','ganso','pavao','frango_corte','galinha_caipira'].includes(type)) ? 1.1 : 1.0;
  ...
  const feedAnimal = (id: number, event: React.MouseEvent) => {
    ...
    else if (animal.type === 'ovelha' || animal.type === 'cabra' || animal.type === 'cabra_angora' || animal.type === 'cabra_boer' || animal.type === 'lhama' || animal.type === 'alpaca') { feedType = 'racaoOvinos'; feedLabel = 'Ração de Ovinos'; }
    ...
    else if (animal.type === 'tanque_tilapia' || animal.type === 'tanque_pirarucu') { feedType = 'racaoPeixe' as any; feedLabel = 'Ração de Peixes'; }
  ...
  const collectEgg = (id: number, event: React.MouseEvent) => {
    ...
    if (animal.type === 'galinha_caipira') {
      ...
      setStats(prev => ({ ...prev, totalCollected: prev.totalCollected + 1, totalEggs: ((prev as any).totalEggs || 0) + 1 }));
      setAnimals(prev => prev.map(a => a.id === id ? { ...a, hasProducedToday: false, daysSinceLastEgg: 0 } : a));
      addLog(`🥚 ${animal.name} rendeu 1 Ovo Caipira premium!`, 'success');
  ...
  // Collect Sheep Milk (Ovelha Leiteira)
  const collectSheepMilk = (id: number, event: React.MouseEvent) => {
    if (event) event.preventDefault();
    const animal = animals.find(a => a.id === id);
    if (!animal || animal.type !== 'ovelha_leiteira') return;
    ...
    setInventory(prev => ({ ...prev, sheep_milk: (prev.sheep_milk ?? 0) + qty }));
    setProductFreshness((prev: any) => ({ ...prev, sheep_milk: 3 }));
    setStats(prev => ({ ...prev, totalCollected: prev.totalCollected + qty }));
    setAnimals(prev => prev.map(a => a.id === id ? { ...a, hasProducedToday: false } : a));
    addLog(`🐑 ${animal.name} produziu ${qty} leite(s) de ovelha!`, 'success');
    triggerAudioResult(() => sfx.playSound('collect'));
    spawnFeedback('🥛', `+${qty} Leite Ovelha`, event);
    updateMissionProgress('collect_items', qty);
    onItemCollected?.(qty);
  };
  ...
  // Collect Cabra Angorá Mohair
  const collectCabraAngoraMohair = (id: number, event: React.MouseEvent) => {
    if (event) event.preventDefault();
    const animal = animals.find(a => a.id === id);
    if (!animal || animal.type !== 'cabra_angora') return;
    if (!animal.woolReady) { addLog(`🐐 ${animal.name} ainda não está pronta para tosquia!`, 'error'); spawnFeedback('⏳', 'Aguarde', event); return; }
    const qty = specialization === 'fibras' ? 2 : 1;
    setInventory(prev => ({ ...prev, mohair: (prev.mohair ?? 0) + qty }));
    setAnimals(prev => prev.map(a => a.id === id ? { ...a, woolReady: false, daysSinceLastWool: 0 } : a));
    addLog(`🐐 ${animal.name} (Cabra Angorá) tosquiada! +${qty} mohair.`, 'success');
    setFarmXp(prev => prev + qty * 2);
    triggerAudioResult(() => sfx.playSound('collect'));
    spawnFeedback('🧶', `+${qty} Mohair`, event);
    onItemCollected?.(qty);
    setTimeout(() => checkAndUnlockAchievement('mohair_collector'), 0);
  };
  ...
  // 5f. Boi Angus — carne premium: valor pesa marmoreio (peso) e bem-estar (felicidade)
  const calculateAngusValue = (angus: Animal): number => {
  ...
  const sellAngus = (id: number, event: React.MouseEvent) => {
    if (event) event.preventDefault();
    const animal = animals.find(a => a.id === id);
    if (!animal || animal.type !== 'boi_angus') return;

    const value = Math.floor(calculateAngusValue(animal) * (getFreightMultiplier ? getFreightMultiplier('animais') : 1));

    setGold(prev => prev + value);
    setDailyEarning(prev => prev + value);
    addFinancialEntry?.({ day: currentDay, type: 'income', amount: value, category: 'venda', description: `Venda: ${animal.name} (Boi Angus)` });
    setStats(prev => ({
      ...prev,
      totalEarned: prev.totalEarned + value,
      totalSold: prev.totalSold + 1,
  ...
  // 5f2. Cabra Boer — raça de corte: cresce rápido, valor pesa peso e bem-estar
  const calculateBoerValue = (boer: Animal): number => {
  ...
  const sellBoer = (id: number, event: React.MouseEvent) => {
    if (event) event.preventDefault();
    const animal = animals.find(a => a.id === id);
    if (!animal || animal.type !== 'cabra_boer') return;
    if (animal.isAdult === false) {
      addLog('🐐 O cabrito precisa crescer antes de ser vendido!', 'error');
      return;
    }

    const value = Math.floor(calculateBoerValue(animal) * (getFreightMultiplier ? getFreightMultiplier('animais') : 1));

    setGold(prev => prev + value);
    setDailyEarning(prev => prev + value);
    addFinancialEntry?.({ day: currentDay, type: 'income', amount: value, category: 'venda', description: `Venda: ${animal.name} (Cabra Boer)` });
    setStats(prev => ({
      ...prev,
      totalEarned: prev.totalEarned + value,
      totalSold: prev.totalSold + 1,
    ...
    setAnimals(prev => prev.filter(a => a.id !== id));
    updateMissionProgress('sell_livestock', 1);
    addLog(`💰 ${animal.name} (Cabra Boer) foi vendida na feira por ${value} moedas!`, 'success');
    triggerAudioResult(() => sfx.playSound('sell_animal'));
    triggerConfetti(event);
    spawnFeedback('💰', `+${value} 💰`, event);
  };

  // 5f3. Tanque de Pirarucu — peixe amazônico premium: cresce rápido, vendido de uma vez por peso
  // Valor calibrado pra ficar ~65% do teto do Boi Angus (item mais valioso do jogo) — o Pirarucu
  // é caríssimo por kg na vida real, mas é um peixe de ~10-15kg, bem menor que um boi inteiro.
  const calculatePirarucuValue = (pirarucu: Animal): number => {
  ...
  const sellPirarucu = (id: number, event: React.MouseEvent) => {
    if (event) event.preventDefault();
    const animal = animals.find(a => a.id === id);
    if (!animal || animal.type !== 'tanque_pirarucu') return;
    if (animal.isAdult === false) {
      addLog('🐋 O pirarucu ainda precisa engordar antes da despesca!', 'error');
      return;
    }

    const value = Math.floor(calculatePirarucuValue(animal) * (getFreightMultiplier ? getFreightMultiplier('animais') : 1));

    setGold(prev => prev + value);
    setDailyEarning(prev => prev + value);
    addFinancialEntry?.({ day: currentDay, type: 'income', amount: value, category: 'venda', description: `Venda: ${animal.name} (Pirarucu)` });
    setStats(prev => ({
      ...prev,
      totalEarned: prev.totalEarned + value,
      totalSold: prev.totalSold + 1,
    ...
    setAnimals(prev => prev.filter(a => a.id !== id));
    updateMissionProgress('sell_livestock', 1);
    addLog(`💰 ${animal.name} (Pirarucu) foi despescado e vendido por ${value} moedas!`, 'success');
    triggerAudioResult(() => sfx.playSound('sell_animal'));
    triggerConfetti(event);
    spawnFeedback('💰', `+${value} 💰`, event);
  };
  ...
  const collectPeixe = (id: number, event: React.MouseEvent) => {
    ...
    if (!animal.fishReady) {
      addLog('🐟 As tilápias ainda estão crescendo!', 'error');
  ...
  const sellFrango = (id: number, event: React.MouseEvent) => {
    if (event) event.preventDefault();
    const animal = animals.find(a => a.id === id);
    if (!animal || animal.type !== 'frango_corte') return;

    const value = Math.floor(calculateFrangoValue(animal) * (getFreightMultiplier ? getFreightMultiplier('animais') : 1));

    setGold(prev => prev + value);
    setDailyEarning(prev => prev + value);
    addFinancialEntry?.({ day: currentDay, type: 'income', amount: value, category: 'venda', description: `Venda: ${animal.name} (Frango de Corte)` });
    setStats(prev => ({
      ...prev,
      totalEarned: prev.totalEarned + value,
      totalSold: prev.totalSold + 1,
  ...
  // 5b. Peru (Turkey)
  const calculatePeruValue = (peru: Animal): number => {
    const growthFactor = peru.weightGain || 0.0;
    let base = 80 + (growthFactor * 470); // 80 (0%) → 550 (100%)
    let happinessBonus = (peru.happiness / 100) * 50; // Max 50 bonus
    let finalValueBase = Math.floor(base + happinessBonus);
  ...
  const sellPeru = (id: number, event: React.MouseEvent) => {
    if (event) event.preventDefault();
    const animal = animals.find(a => a.id === id);
    if (!animal || animal.type !== 'peru') return;

    const value = Math.floor(calculatePeruValue(animal) * (getFreightMultiplier ? getFreightMultiplier('animais') : 1));

    setGold(prev => prev + value);
    setDailyEarning(prev => prev + value);
    addFinancialEntry?.({ day: currentDay, type: 'income', amount: value, category: 'venda', description: `Venda: ${animal.name} (Peru)` });
    setStats(prev => ({
      ...prev,
      totalEarned: prev.totalEarned + value,
      totalSold: prev.totalSold + 1,
    ...
    setAnimals(prev => prev.filter(a => a.id !== id));
    addLog(`💰 ${animal.name} (Peru) foi vendido na feira por ${value} moedas!`, 'success');
    setTimeout(() => checkAndUnlockAchievement?.('peru_seller'), 0);
  ...
  const buyAnimal = (type: AnimalType, event: React.MouseEvent) => {
    ...
    const name = type === 'boi' ? getUniqueOxName(animals) : type === 'frango_corte' ? getUniqueFrangoName(animals) : type === 'boi_angus' ? getUniqueAngusName(animals) : type === 'cabra_boer' ? getUniqueBoerName(animals) : type === 'cavalo' ? getUniqueCavaloName(animals) : type === 'porco' ? getUniquePorcoName(animals) : type === 'minhoca' ? 'Minhocário' : type === 'caracol' ? 'Criatório de Caracóis' : type === 'colmeia_abelhas' ? 'Colmeia de Abelhas' : type === 'tanque_tilapia' ? 'Tanque de Tilápia' : type === 'tanque_pirarucu' ? 'Tanque de Pirarucu' : getRandomName(type);

    // Custom initial stats
    const happiness = Math.floor(Math.random() * 21) + 60; // between 60 and 80

    // F1: maxAge por tipo com variação ±20%
    const baseMaxAgeMap: Record<string, number> = { vaca: 120, ovelha: 90, boi: 150, galinha: 60, cabra: 200, lhama: 180, pato: 80, ganso: 150, bufalo: 220, pavao: 160, codorna: 60, alpaca: 180, minhoca: 365, caracol: 200, coelho_angora: 100, bicho_seda: 20, ra: 120, avestruz: 365, jacare: 400, porco: 120, colmeia_abelhas: 9999, ovelha_leiteira: 180, peru: 150, cabra_angora: 150, frango_corte: 45, galinha_caipira: 70, tanque_tilapia: 9999, vaca_jersey: 140, boi_angus: 160, cavalo: 160, cabra_boer: 100, tanque_pirarucu: 9999 };
    const baseMaxAge = baseMaxAgeMap[type] ?? 90;
    const variation = (type === 'colmeia_abelhas' || type === 'tanque_tilapia' || type === 'tanque_pirarucu') ? 1 : 1 + (Math.random() * 0.4 - 0.2);
    ...
    const newAnimal: Animal = {
      ...
      ...(type === 'cabra_boer' && { weightGain: 0.08 }),
      ...(type === 'tanque_pirarucu' && { weightGain: 0.15, trait: undefined }),
      ...
      ...(type === 'cabra_angora' && { daysUntilWool: 25, daysSinceLastWool: 0, woolReady: false }),
    ...
    else if (type === 'peru') typeLabel = '🦃 Peru';
    else if (type === 'cabra_angora') typeLabel = '🐐 Cabra Angorá';
  ...
  const FILHOTE_CONFIG: Partial<Record<AnimalType, { price: number; daysToAdult: number }>> = {
    ...
    ovelha_leiteira: { price: 90, daysToAdult: 10 },
  ...
  const buyAnimalFilhote = (type: AnimalType, event: React.MouseEvent) => {
    ...
    const newFilhote: Animal = {
      ...
      ...(type === 'ovelha_leiteira' && { isLactating: false, lactationCycle: 0 }),
      ...(type === 'lhama' && { woolAccumulated: 0 }),
    ...
    const typeLabel = type === 'vaca' ? '🐄 Vaca' : type === 'boi' ? '🐂 Boi' : type === 'bufalo' ? '🐃 Búfalo' : type === 'cabra' ? '🐐 Cabra' : type === 'ovelha' ? '🐑 Ovelha' : type === 'ovelha_leiteira' ? '🐑 Ovelha Leiteira' : type === 'pato' ? '🦆 Pato' : '🦚 Pavão';
