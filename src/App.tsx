...
// Custo de água por tipo de animal (fora do componente — valor estático, sem recriação por render)
...
const [currentScreen, setCurrentScreen] = useState<'splash' | 'title' | 'game'>('splash');
  ...
  const processarFomeFelicidade = (
    ...
    return animalsList.map(animal => {
      ...
      if (copy.type === 'vaca') {
        ...
        // Caipira: bota dia sim, dia não, e só com felicidade alta (85+)
        copy.daysSinceLastEgg = (copy.daysSinceLastEgg ?? 0) + 1;
        const cooldownOk = (copy.daysSinceLastEgg ?? 0) >= 2;
        const canProduce = copy.hunger > 25 && copy.happiness >= 85 && !sickProductionBlock && cooldownOk;
        copy.hasProducedToday = canProduce;
        if (canProduce) {
          logs.push({ msg: `🐔 ${copy.name} botou um precioso ovo caipira!`, type: 'success' });
        } else if (cooldownOk && copy.happiness < 85 && copy.hunger > 25) {
          logs.push({ msg: `🐔 ${copy.name} precisa de felicidade 85+ para botar ovo caipira (atual: ${Math.floor(copy.happiness)}%).`, type: 'error' });
        }
      }
        ...
        const adjustedPhaseMult = Math.min(1.15, basePhaseMult + (copy.isVeteran ? 0.05 : 0) + (copy.juvenileBonus || 0));
        const lifePhaseBlock = adjustedPhaseMult < 1.0 && Math.random() > adjustedPhaseMult;
        const canProduce = copy.isAdult !== false && copy.isLactating !== false && copy.hunger > 25 && copy.happiness > 30 && !sickProductionBlock && !lifePhaseBlock;
        copy.hasProducedToday = canProduce;
        if (canProduce) {
  ...
  const LONG_CONTRACT_CATALOG = [
    ...
    { catalogId: 'lc_14', client: 'Tecelagem do Vale Encantado', product: 'llama_wool' as const, description: 'Cooperativa de artesãos que produz mantas, tapetes e peças decorativas vendidas para lojas de design e decoração do Sudeste. A lã de lhama é mais macia e mais leve que a ovina — ideal para peças de alto valor agregado. Eles precisam de regularidade para manter o ritmo de produção.', baseMarket: 28, pricePerUnit: 65, weeklyGoal: 4, durationDays: 120, minLevel: 7, completionBonus: 670, completionXP: 100 },
    { catalogId: 'lc_np1', client: 'Empório da Vila', product: 'ovo_caipira' as const, description: 'Um empório de produtos coloniais que só trabalha com ovos caipiras de verdade — casca firme, gema alaranjada. O dono prova um ovo de cada lote antes de aceitar a entrega. Exigente, mas paga o justo pelo premium.', baseMarket: 10, pricePerUnit: 13, weeklyGoal: 6, durationDays: 60, minLevel: 5, completionBonus: 160, completionXP: 30 },
    { catalogId: 'lc_np2', client: 'Restaurante do Porto', product: 'peixe' as const, description: 'Restaurante à beira do rio famoso pela tilápia fresca. O chef faz questão de peixe do dia, criado em água limpa. Entregas semanais pontuais mantêm o cardápio — e a reputação de vocês dois.', baseMarket: 45, pricePerUnit: 55, weeklyGoal: 5, durationDays: 60, minLevel: 12, completionBonus: 280, completionXP: 35 },
    { catalogId: 'lc_np3', client: 'Confeitaria Doce Minas', product: 'bolo_caipira' as const, description: 'Confeitaria que virou febre na cidade com fatias de bolo caipira servidas no café da tarde. A receita da vovó com ovos premium não pode faltar na vitrine — dois bolos por semana, sagrado.', baseMarket: 50, pricePerUnit: 62, weeklyGoal: 2, durationDays: 60, minLevel: 6, completionBonus: 240, completionXP: 35 },
    { catalogId: 'lc_np4', client: 'Boutique de Queijos Alvorada', product: 'queijo_minas_jersey' as const, description: 'Boutique especializada em queijos de leite Jersey — mais gordo, mais sabor. Clientela fiel que paga caro pelo frescal autêntico. Um contrato de prestígio para queijeiros de elite.', baseMarket: 95, pricePerUnit: 115, weeklyGoal: 2, durationDays: 60, minLevel: 15, completionBonus: 420, completionXP: 45 },
    { catalogId: 'lc_31', client: 'Confeitaria Doce Memória', product: 'leite_condensado' as const, description: 'Confeitaria artesanal conhecida pelos doces de festa, bolos decorados e brigadeiros gourmet vendidos para eventos e encomendas. O leite condensado entra em quase tudo que sai da cozinha — brigadeiro, beijinho, fudge, recheio de bolo e muito mais. A confeiteira já tentou produzir com produto de caixinha e clientes notaram a diferença. Agora ela busca fornecimento artesanal fixo, com entrega semanal antes do fim de semana de maior movimento.', baseMarket: 145, pricePerUnit: 185, weeklyGoal: 3, durationDays: 120, minLevel: 7, completionBonus: 1100, completionXP: 110 },
    // --- Nível 8 ---
    { catalogId: 'lc_15', client: 'Laticínios Premium do Norte', product: 'buffalo_milk' as const, description: 'Empresa especializada em derivados de búfala para o mercado gourmet. O leite serve de base para mozzarella, ricota e iogurte premium comercializados em delicatessens de todo o país. Eles pagam acima do mercado porque sabem que poucos produtores têm búfalas em produção estável.', baseMarket: 35, pricePerUnit: 42, weeklyGoal: 5, durationDays: 120, minLevel: 8, completionBonus: 540, completionXP: 95 },
    { catalogId: 'lc_16', client: 'Exportadora Fibras Raras', product: 'alpaca_wool' as const, description: 'Empresa que processa e exporta fibras especiais para marcas de moda italiana e portuguesa. A lã de alpaca é classificada como fibra nobre — levíssima, hipoalergênica, com valor crescente no mercado europeu. Contrato exigente em qualidade, mas com remuneração muito acima da média nacional.', baseMarket: 75, pricePerUnit: 92, weeklyGoal: 4, durationDays: 120, minLevel: 8, completionBonus: 940, completionXP: 120 },
    ...
    // --- Ovelha Leiteira ---
    { catalogId: 'lc_sheep1', client: 'Queijaria Bela Canastra', product: 'sheep_milk' as const, description: 'Queijaria familiar de terceira geração, especializada em queijos de leite de ovelha seguindo receitas trazidas da Serra da Canastra. Eles precisam de leite fresco toda semana para manter o ritmo da produção — e são exigentes com a procedência. Não aceitam leite de tambor industrial. Querem saber o nome da fazenda.', baseMarket: 28, pricePerUnit: 38, weeklyGoal: 5, durationDays: 90, minLevel: 5, completionBonus: 420, completionXP: 75 },
    { catalogId: 'lc_sheep2', client: 'Café & Bistrô Mirante', product: 'iogurte_ovelha' as const, description: 'Café boutique situado em uma estrada panorâmica que atrai turistas e ciclistas nos fins de semana. O cardápio tem um toque europeu — e o iogurte de ovelha é servido com mel e granola no café da manhã especial. Já tentaram com iogurte de vaca, mas os clientes notaram a diferença e pediram de volta o de ovelha.', baseMarket: 80, pricePerUnit: 105, weeklyGoal: 3, durationDays: 120, minLevel: 6, completionBonus: 580, completionXP: 90 },
    { catalogId: 'lc_sheep3', client: 'Trattoria dell\'Orto', product: 'ricota_ovelha' as const, description: 'Restaurante italiano de cozinha autêntica que importa parte dos insumos mas tenta usar laticínios locais sempre que a qualidade permite. A ricota de ovelha entra em massas frescas, raviólis e sobremesas — e o chef é categórico: ricota de ovelha ou não serve. Volume modesto, mas o prestígio da casa abre portas.', baseMarket: 95, pricePerUnit: 125, weeklyGoal: 3, durationDays: 120, minLevel: 7, completionBonus: 680, completionXP: 105 },
    { catalogId: 'lc_sheep4', client: 'Confeitaria Nata & Mel', product: 'doce_leite_ovelha' as const, description: 'Confeitaria de alto padrão que vende doces finos para eventos, casamentos e cestas gourmet. O doce de leite de ovelha é mais cremoso e com sabor mais complexo que o convencional — virou o carro-chefe da linha premium deles. A demanda cresceu mais rápido que a oferta. Procuram fornecedor fixo e confiável.', baseMarket: 160, pricePerUnit: 205, weeklyGoal: 2, durationDays: 120, minLevel: 8, completionBonus: 820, completionXP: 120 },
    { catalogId: 'lc_sheep5', client: 'Empório Origens Raras', product: 'queijo_pecorino' as const, description: 'Empório especializado em produtos de origem controlada — queijos, embutidos e azeites que não se acham no mercado comum. O queijo pecorino artesanal brasileiro é uma raridade que eles querem exclusividade de exibição nas prateleiras. Pagam acima da média e exigem maturação mínima — mas a vitrine vale o esforço.', baseMarket: 280, pricePerUnit: 360, weeklyGoal: 2, durationDays: 150, minLevel: 9, completionBonus: 1200, completionXP: 150 },
    // --- Novos fornecedores: cobertura completa de produtos + segundas opções ---
    { catalogId: 'lc_g01', client: 'Empório Jersey & Cia', product: 'leite_jersey' as const, description: 'Casa de produtos finos que descobriu o leite Jersey e não quer outro: mais gordura, mais sabor, mais clientes voltando. Querem fornecimento semanal fixo para a linha de cafés especiais e cremes da casa.', baseMarket: 14, pricePerUnit: 18, weeklyGoal: 6, durationDays: 60, minLevel: 15, completionBonus: 130, completionXP: 35 },
    { catalogId: 'lc_g02', client: 'Confeitaria Flor de Lis', product: 'manteiga_jersey' as const, description: 'Confeitaria de bairro famosa pelos croissants folhados. A chef jura que a manteiga Jersey é o segredo da massa — e paga a mais para não deixar o forno esperando.', baseMarket: 60, pricePerUnit: 78, weeklyGoal: 3, durationDays: 60, minLevel: 15, completionBonus: 280, completionXP: 35 },
    { catalogId: 'lc_g03', client: 'Doceria Vovó Mercedes', product: 'doce_leite_jersey' as const, description: 'Doceria de tradição familiar, três gerações atrás do mesmo tacho de cobre. O doce de leite Jersey virou o carro-chefe das caixas de presente — produção pequena, exigência enorme.', baseMarket: 120, pricePerUnit: 156, weeklyGoal: 2, durationDays: 60, minLevel: 15, completionBonus: 375, completionXP: 35 },
    { catalogId: 'lc_g04', client: 'Queijaria Vila Holanda', product: 'gouda_jersey' as const, description: 'Casa de queijos especiais que monta tábuas para eventos e vinícolas. O Gouda de leite Jersey maturado é a estrela do catálogo — eles sabem que leva dias para ficar pronto e pagam pela paciência.', baseMarket: 200, pricePerUnit: 260, weeklyGoal: 1, durationDays: 120, minLevel: 15, completionBonus: 310, completionXP: 60 },
    { catalogId: 'lc_g05', client: 'Café da Serra Azul', product: 'iogurte_bufala' as const, description: 'Cafeteria de estrada com fila aos domingos. O iogurte de búfala entra nas taças de granola e frutas do café da manhã — cremosidade que o cliente percebe na primeira colherada.', baseMarket: 90, pricePerUnit: 117, weeklyGoal: 3, durationDays: 60, minLevel: 5, completionBonus: 420, completionXP: 35 },
    { catalogId: 'lc_g06', client: 'Padaria Pão da Vila', product: 'manteiga_bufala' as const, description: 'Padaria artesanal que assa em forno a lenha. A manteiga de búfala vai na massa das brioches e no balcão, em porções embrulhadas à mão. Volume constante, sem sustos.', baseMarket: 110, pricePerUnit: 143, weeklyGoal: 3, durationDays: 60, minLevel: 5, completionBonus: 515, completionXP: 35 },
    { catalogId: 'lc_g07', client: 'Doces do Cerrado', product: 'doce_leite_bufala' as const, description: 'Fábrica artesanal de doces regionais que vende para empórios de três estados. O doce de leite de búfala é a edição especial do catálogo — denso, raro e disputado.', baseMarket: 250, pricePerUnit: 320, weeklyGoal: 2, durationDays: 90, minLevel: 6, completionBonus: 770, completionXP: 35 },
    { catalogId: 'lc_g08', client: 'Trattoria Nonna Lucia', product: 'burrata' as const, description: 'Cantina italiana onde a burrata chega à mesa ainda fria, com tomate e manjericão da horta. A nonna prova cada remessa pessoalmente — e devolve o que não estiver perfeito.', baseMarket: 380, pricePerUnit: 494, weeklyGoal: 1, durationDays: 90, minLevel: 8, completionBonus: 595, completionXP: 60 },
    { catalogId: 'lc_g09', client: 'Cantina Piemonte', product: 'queijo_parmesao' as const, description: 'Restaurante de massas que rala parmesão na frente do cliente. Maturação de verdade leva semanas, eles sabem — o contrato é longo justamente para respeitar o tempo do queijo.', baseMarket: 200, pricePerUnit: 260, weeklyGoal: 1, durationDays: 120, minLevel: 11, completionBonus: 310, completionXP: 90 },
    { catalogId: 'lc_g10', client: 'Armazém da Montanha', product: 'queijo_serra' as const, description: 'Empório de altitude especializado em queijos de guarda. O Queijo da Serra maturado é vendido por fatia numerada, com origem estampada no rótulo. O contrato mais paciente do vale — e um dos que melhor pagam.', baseMarket: 280, pricePerUnit: 364, weeklyGoal: 1, durationDays: 150, minLevel: 15, completionBonus: 435, completionXP: 120 },
    { catalogId: 'lc_g11', client: 'Granja Incubadora Santa Fé', product: 'fertile_egg' as const, description: 'Incubadora regional que fornece pintinhos para pequenos criadores. Ovos férteis de galinhas felizes têm taxa de eclosão muito maior — eles pagam prêmio por cada um que chegar.', baseMarket: 36, pricePerUnit: 49, weeklyGoal: 2, durationDays: 60, minLevel: 3, completionBonus: 120, completionXP: 25 },
    { catalogId: 'lc_g12', client: 'Bistrô Le Canard', product: 'pate_pato' as const, description: 'Bistrô de inspiração francesa com carta de vinhos premiada. O patê de pato abre o menu-degustação — o chef exige receita artesanal e entrega pontual toda semana.', baseMarket: 210, pricePerUnit: 273, weeklyGoal: 2, durationDays: 90, minLevel: 5, completionBonus: 655, completionXP: 35 },
    { catalogId: 'lc_g13', client: 'Boteco do Trilho', product: 'ovo_defumado' as const, description: 'Boteco famoso pelos petiscos defumados que acompanham o chope. O ovo defumado da fazenda virou lenda no balcão — eles querem garantia de estoque para o fim de semana.', baseMarket: 120, pricePerUnit: 156, weeklyGoal: 3, durationDays: 60, minLevel: 13, completionBonus: 560, completionXP: 35 },
    { catalogId: 'lc_g14', client: 'Empório Mineiro Sabor & Arte', product: 'conserva_codorna' as const, description: 'Empório de conservas artesanais que monta cestas de presente. Os vidrinhos de codorna em conserva saem aos pares — apresentação impecável e prazo religioso.', baseMarket: 160, pricePerUnit: 208, weeklyGoal: 2, durationDays: 60, minLevel: 11, completionBonus: 500, completionXP: 35 },
    { catalogId: 'lc_g15', client: 'Cafeteria Quintal da Roça', product: 'pudim_caipira' as const, description: 'Cafeteria que aposta em sobremesas de fazenda de verdade. O pudim de ovo caipira esgota antes das quatro da tarde — a dona quer fornecimento fixo para parar de decepcionar cliente.', baseMarket: 35, pricePerUnit: 46, weeklyGoal: 3, durationDays: 60, minLevel: 5, completionBonus: 165, completionXP: 35 },
    { catalogId: 'lc_g16', client: 'Adega & Tábuas Beira Rio', product: 'peixe_defumado' as const, description: 'Casa de vinhos que serve tábuas de defumados. A tilápia defumada da fazenda combina com os brancos da casa — pedem regularidade para manter o item no cardápio fixo.', baseMarket: 70, pricePerUnit: 91, weeklyGoal: 3, durationDays: 60, minLevel: 12, completionBonus: 330, completionXP: 35 },
    { catalogId: 'lc_g17', client: 'Buffet Dona Iracema', product: 'bolinho_peixe' as const, description: 'Buffet de festas que serve casamentos na região inteira. O bolinho de peixe é o salgado mais pedido das bandejas — cada evento consome dúzias, e a agenda está cheia.', baseMarket: 85, pricePerUnit: 110, weeklyGoal: 3, durationDays: 60, minLevel: 12, completionBonus: 395, completionXP: 35 },
    { catalogId: 'lc_g18', client: 'Restaurante Maré Alta', product: 'moqueca' as const, description: 'Restaurante de cozinha litorânea a quilômetros do mar — o segredo é a moqueca da fazenda, feita com tilápia fresca e leite da casa. Prato assinatura, fornecedor único.', baseMarket: 160, pricePerUnit: 208, weeklyGoal: 2, durationDays: 90, minLevel: 12, completionBonus: 500, completionXP: 60 },
    { catalogId: 'lc_g19', client: 'Mercearia Rota do Interior', product: 'conserva_peixe' as const, description: 'Mercearia de estrada que abastece viajantes e sitiantes. A conserva de peixe tem validade longa e giro rápido na prateleira — contrato simples, pagamento honesto.', baseMarket: 140, pricePerUnit: 182, weeklyGoal: 2, durationDays: 60, minLevel: 12, completionBonus: 435, completionXP: 35 },
    { catalogId: 'lc_g20', client: 'Brechó de Lã Ponto Firme', product: 'scarf' as const, description: 'Ateliê de tricô que revende peças de lã no inverno e ensina curso no verão. Cachecóis simples e bem feitos nunca encalham — eles compram tudo o que a fazenda tecer.', baseMarket: 50, pricePerUnit: 65, weeklyGoal: 3, durationDays: 60, minLevel: 2, completionBonus: 235, completionXP: 25 },
    { catalogId: 'lc_g21', client: 'Armarinho Central', product: 'fio_lhama' as const, description: 'Armarinho tradicional do centro da cidade, balcão de madeira e novelos até o teto. O fio de lhama é o mais macio da loja — as tricoteiras brigam pelos últimos rolos.', baseMarket: 45, pricePerUnit: 58, weeklyGoal: 4, durationDays: 60, minLevel: 3, completionBonus: 280, completionXP: 25 },
    { catalogId: 'lc_g22', client: 'Loja Vento Sul', product: 'cachecol_lhama' as const, description: 'Loja de roupas de frio na serra, movimento explode na temporada. O cachecol de lhama é presente certo de turista — eles querem estoque garantido antes do inverno.', baseMarket: 80, pricePerUnit: 104, weeklyGoal: 2, durationDays: 60, minLevel: 4, completionBonus: 250, completionXP: 25 },
    { catalogId: 'lc_g23', client: 'Chapelaria do Vale', product: 'gorro_lhama' as const, description: 'Chapelaria centenária que se reinventou com acessórios de fibras naturais. O gorro de lhama é o item que trouxe a clientela jovem de volta à loja.', baseMarket: 75, pricePerUnit: 98, weeklyGoal: 2, durationDays: 60, minLevel: 4, completionBonus: 235, completionXP: 25 },
    { catalogId: 'lc_g24', client: 'Montanhismo Pico Alto', product: 'luvas_lhama' as const, description: 'Loja de equipamentos de trilha e montanha. Luvas de lhama aquecem sem pesar na mochila — os guias da região recomendam, e a prateleira vive vazia.', baseMarket: 70, pricePerUnit: 91, weeklyGoal: 2, durationDays: 60, minLevel: 5, completionBonus: 220, completionXP: 35 },
    { catalogId: 'lc_g25', client: 'Ateliê Andino', product: 'poncho_lhama' as const, description: 'Ateliê de moda artesanal inspirado nos Andes. O poncho de lhama é peça de vitrine, fotografada e postada — cada remessa nova esgota em dias.', baseMarket: 140, pricePerUnit: 182, weeklyGoal: 2, durationDays: 90, minLevel: 6, completionBonus: 435, completionXP: 35 },
    { catalogId: 'lc_g26', client: 'Pousada Recanto da Neblina', product: 'manta_lhama' as const, description: 'Pousada romântica na serra onde cada quarto tem lareira e manta de lhama aos pés da cama. Os hóspedes tanto insistiram em comprar que agora a pousada revende.', baseMarket: 200, pricePerUnit: 260, weeklyGoal: 1, durationDays: 90, minLevel: 7, completionBonus: 310, completionXP: 35 },
    { catalogId: 'lc_g27', client: 'Alfaiataria Fio Nobre', product: 'mohair' as const, description: 'Alfaiataria que trabalha com fibras nobres importadas — até descobrir o mohair da fazenda. Produção lenta, eles sabem: o contrato é longo e o preço respeita a raridade.', baseMarket: 120, pricePerUnit: 156, weeklyGoal: 1, durationDays: 120, minLevel: 9, completionBonus: 185, completionXP: 60 },
    { catalogId: 'lc_g28', client: 'Boutique Inverno Europeu', product: 'cachecol_mohair' as const, description: 'Boutique de acessórios de luxo que atende clientela exigente. O cachecol de mohair é vendido em caixa forrada, com certificado de origem da fazenda. Peça rara, margem alta.', baseMarket: 280, pricePerUnit: 364, weeklyGoal: 1, durationDays: 120, minLevel: 10, completionBonus: 435, completionXP: 60 },
    { catalogId: 'lc_g29', client: 'Spa Águas Claras', product: 'creme_cosmetico' as const, description: 'Spa de luxo em meio à mata que só usa cosméticos naturais. O creme de muco de caracol é o tratamento mais reservado da casa — fornecimento discreto e constante.', baseMarket: 220, pricePerUnit: 286, weeklyGoal: 2, durationDays: 60, minLevel: 8, completionBonus: 685, completionXP: 60 },
    { catalogId: 'lc_g30', client: 'Clínica Derma Naturale', product: 'serum_facial' as const, description: 'Clínica de estética que substituiu importados pelo sérum natural da fazenda. As clientes notaram a diferença — a clínica quer exclusividade regional e paga por ela.', baseMarket: 220, pricePerUnit: 286, weeklyGoal: 2, durationDays: 90, minLevel: 8, completionBonus: 685, completionXP: 60 },
    { catalogId: 'lc_g31', client: 'Estúdio Beleza Raiz', product: 'mascara_facial' as const, description: 'Estúdio de beleza focado em produtos veganos e naturais. A máscara facial de muco entra no ritual completo do estúdio — cada sessão consome uma unidade.', baseMarket: 180, pricePerUnit: 234, weeklyGoal: 2, durationDays: 60, minLevel: 9, completionBonus: 560, completionXP: 60 },
    { catalogId: 'lc_g32', client: 'Empório Banho & Aroma', product: 'sabonete_natural' as const, description: 'Empório de sabonetes e aromas artesanais. O sabonete de muco, mel e leite de cabra é o mais vendido da cesta de presente — perfumado, cremoso e com história pra contar.', baseMarket: 180, pricePerUnit: 234, weeklyGoal: 2, durationDays: 60, minLevel: 9, completionBonus: 560, completionXP: 60 },
    { catalogId: 'lc_g33', client: 'Horta Comunitária Boa Terra', product: 'humus' as const, description: 'Horta comunitária que abastece a feira orgânica da cidade. O húmus do minhocário é o adubo que faz as alfaces crescerem de dar orgulho — pedem sacas toda semana.', baseMarket: 22, pricePerUnit: 30, weeklyGoal: 4, durationDays: 60, minLevel: 6, completionBonus: 145, completionXP: 35 },
    { catalogId: 'lc_g34', client: 'Pesqueiro do Tião', product: 'minhoca_viva' as const, description: 'Pesqueiro de lagoa com movimento forte nos fins de semana. Minhoca viva é a isca que os clientes mais pedem — o Tião compra tudo e ainda reclama que é pouco.', baseMarket: 18, pricePerUnit: 24, weeklyGoal: 2, durationDays: 60, minLevel: 7, completionBonus: 60, completionXP: 35 },
    { catalogId: 'lc_g35', client: 'Viveiro Verde Vivo', product: 'biofertilizante' as const, description: 'Viveiro de mudas nativas e frutíferas. O biofertilizante da fazenda substituiu os químicos de vez — as mudas pegam melhor e o viveiro fideliza os sitiantes da região.', baseMarket: 90, pricePerUnit: 117, weeklyGoal: 2, durationDays: 60, minLevel: 7, completionBonus: 280, completionXP: 35 },
    { catalogId: 'lc_g36', client: 'Casa de Chá Hortelã', product: 'mel' as const, description: 'Casa de chá delicada, prateleiras de vidro e bules de porcelana. O mel cru da fazenda adoça os chás da tarde e sai em potinhos no balcão — doçura com procedência.', baseMarket: 80, pricePerUnit: 104, weeklyGoal: 3, durationDays: 60, minLevel: 4, completionBonus: 375, completionXP: 25 },
    { catalogId: 'lc_g37', client: 'Cantina Funghi & Foglia', product: 'cogumelo' as const, description: 'Cantina especializada em cogumelos frescos, do risoto à grelha. O cogumelo colhido na floresta da fazenda chega no mesmo dia — e o chef monta o cardápio em volta dele.', baseMarket: 35, pricePerUnit: 46, weeklyGoal: 3, durationDays: 60, minLevel: 5, completionBonus: 165, completionXP: 35 },
    { catalogId: 'lc_g38', client: 'Taberna do Alambique', product: 'hidromel' as const, description: 'Taberna rústica de bebidas artesanais, barris à vista e música ao vivo. O hidromel da fazenda é servido em caneca de barro — a casa quer garrafas numeradas toda semana.', baseMarket: 260, pricePerUnit: 338, weeklyGoal: 1, durationDays: 90, minLevel: 9, completionBonus: 405, completionXP: 60 },
    { catalogId: 'lc_g39', client: 'Bistrô Colheita', product: 'risoto_cogumelo' as const, description: 'Bistrô de menu curto que muda com a estação. O risoto de cogumelo pronto da fazenda vira o prato executivo do almoço — sai do fogão direto para a mesa.', baseMarket: 150, pricePerUnit: 195, weeklyGoal: 2, durationDays: 60, minLevel: 6, completionBonus: 470, completionXP: 35 },
    { catalogId: 'lc_g40', client: 'Cantina Universitária Panela Cheia', product: 'sopa_cogumelo' as const, description: 'Cantina movimentada que serve centenas de almoços por dia. A sopa de cogumelo é o item mais pedido nos dias frios — panela grande, contrato simples.', baseMarket: 100, pricePerUnit: 130, weeklyGoal: 2, durationDays: 60, minLevel: 5, completionBonus: 310, completionXP: 35 },
    { catalogId: 'lc_g41', client: 'Rotisseria Bella Massa', product: 'massa_fresca' as const, description: 'Rotisseria de massas frescas feitas à vista do cliente. A massa com ovo de ganso tem cor e liga incomparáveis — a casa quer fornecimento fixo para os domingos.', baseMarket: 78, pricePerUnit: 101, weeklyGoal: 2, durationDays: 60, minLevel: 13, completionBonus: 240, completionXP: 35 },
    { catalogId: 'lc_g42', client: 'Café Colonial Estrada Velha', product: 'pao_rustico' as const, description: 'Café colonial de beira de estrada, mesa farta e fila na porta. O pão rústico da fazenda sai quentinho da cesta — eles pedem fornadas semanais sem falta.', baseMarket: 30, pricePerUnit: 40, weeklyGoal: 4, durationDays: 60, minLevel: 3, completionBonus: 190, completionXP: 25 },
    { catalogId: 'lc_g43', client: 'Creperia Roda de Moinho', product: 'crepe_rustico' as const, description: 'Creperia charmosa em um moinho restaurado. O crepe rústico da fazenda é a base de todo o cardápio — simples, honesto e impossível de substituir.', baseMarket: 35, pricePerUnit: 47, weeklyGoal: 4, durationDays: 60, minLevel: 3, completionBonus: 225, completionXP: 25 },
    { catalogId: 'lc_g44', client: 'Brunch & Cia', product: 'waffle_mel' as const, description: 'Casa de brunch que bomba nos fins de semana. O waffle de mel é o prato mais fotografado do salão — eles querem entrega certa antes de todo sábado.', baseMarket: 90, pricePerUnit: 117, weeklyGoal: 3, durationDays: 60, minLevel: 6, completionBonus: 420, completionXP: 35 },
    { catalogId: 'lc_g45', client: 'Maison Sauvage', product: 'bolsa_exotica' as const, description: 'Maison de acessórios exóticos com clientela internacional. A bolsa de couro de jacaré é peça de coleção, encomendada com meses de antecedência — o contrato mais exclusivo da fazenda.', baseMarket: 820, pricePerUnit: 1066, weeklyGoal: 1, durationDays: 150, minLevel: 20, completionBonus: 1280, completionXP: 120 },
    { catalogId: 'lc_g46', client: 'Cooperativa Leite do Vale', product: 'milk' as const, description: 'Cooperativa grande que abastece escolas e mercados da região. Pagam um pouco menos por litro que a concorrência — mas compram volume alto, toda semana, sem falta.', baseMarket: 8, pricePerUnit: 9, weeklyGoal: 13, durationDays: 90, minLevel: 3, completionBonus: 195, completionXP: 25 },
    { catalogId: 'lc_g47', client: 'Rede Café Aurora', product: 'egg' as const, description: 'Rede de cafeterias em expansão que consome bandejas de ovos por dia. Contrato de volume: meta alta, pagamento certo e a chance de crescer junto com a rede.', baseMarket: 4, pricePerUnit: 5, weeklyGoal: 18, durationDays: 90, minLevel: 3, completionBonus: 150, completionXP: 25 },
    { catalogId: 'lc_g48', client: 'Tecelagem Fio do Campo', product: 'wool' as const, description: 'Tecelagem industrial que produz cobertores para lojas de departamento. Precisam de lã crua em quantidade — menos romance, mais escala.', baseMarket: 15, pricePerUnit: 19, weeklyGoal: 7, durationDays: 90, minLevel: 4, completionBonus: 230, completionXP: 25 },
    { catalogId: 'lc_g49', client: 'Distribuidora Queijo & Prosa', product: 'cheese' as const, description: 'Distribuidora que leva queijo simples para padarias de toda a região. Giro alto e meta puxada — perfeito para quem tem várias vacas no pasto.', baseMarket: 30, pricePerUnit: 38, weeklyGoal: 6, durationDays: 90, minLevel: 4, completionBonus: 365, completionXP: 25 },
    { catalogId: 'lc_g50', client: 'Peixaria Água Doce', product: 'peixe' as const, description: 'Peixaria de mercado municipal com balcão disputado desde cedo. Querem tilápia em volume para a banca do sábado — meta alta, preço justo.', baseMarket: 45, pricePerUnit: 58, weeklyGoal: 7, durationDays: 90, minLevel: 12, completionBonus: 695, completionXP: 60 },
    // --- Exportação Internacional: mesmos produtos da fazenda, preço bem acima do mercado local ---
    { catalogId: 'exp_cn', client: '🇨🇳 Distribuidora Têxtil de Suzhou', product: 'cachecol_mohair' as const, description: 'Suzhou é a capital histórica da seda chinesa, mas os compradores de lá andam de olho em fibras nobres alternativas — e o mohair da fazenda chamou atenção numa feira em Xangai. Um distribuidor fechou um pedido-teste pequeno, prometendo escalar se a qualidade se confirmar peça a peça.', baseMarket: 364, pricePerUnit: 583, weeklyGoal: 1, durationDays: 150, minLevel: 15, completionBonus: 700, completionXP: 90 },
    { catalogId: 'exp_us', client: '🇺🇸 Trattoria Sabatino\'s (Nova York)', product: 'queijo_parmesao' as const, description: 'Uma rede de trattorias italianas em Nova York, cansada da instabilidade nos preços do parmesão importado da Itália, foi atrás de um fornecedor sul-americano de confiança. Provaram o queijo da fazenda numa feira gastronômica e fecharam contrato ali mesmo, sem regatear.', baseMarket: 260, pricePerUnit: 494, weeklyGoal: 1, durationDays: 150, minLevel: 16, completionBonus: 750, completionXP: 100 },
    { catalogId: 'exp_jp', client: '🇯🇵 Mercado de Sashimi Premium (Tóquio)', product: 'peixe' as const, description: 'Uma rede de restaurantes de Tóquio busca peixe de água doce criado em condições impecáveis, pra servir cru, no estilo sashimi. Um comprador japonês visitou a fazenda pessoalmente, testou a água do tanque e aprovou a criação. Pagam premium pela garantia de qualidade — e não toleram atraso.', baseMarket: 57, pricePerUnit: 125, weeklyGoal: 3, durationDays: 120, minLevel: 17, completionBonus: 900, completionXP: 120 },
    ...
    { catalogId: 'exp_pt', client: '🇵🇹 Queijaria Dona Fátima (Serpa)', product: 'goat_milk' as const, description: 'Uma queijaria artesanal em Serpa, no Alentejo, perdeu parte do seu fornecimento local depois de uma seca que reduziu o rebanho da região. A mestre queijeira Dona Fátima busca leite de cabra de qualidade constante para manter viva a receita centenária do queijo Serpa DOP. Ela é exigente com a gordura do leite e testa cada lote antes de aprovar — mas quem entra na lista dela raramente sai.', baseMarket: 16, pricePerUnit: 30, weeklyGoal: 6, durationDays: 90, minLevel: 2, completionBonus: 250, completionXP: 35 },
    { catalogId: 'exp_th', client: '🇹🇭 Mercado de Chiang Mai', product: 'duck_egg' as const, description: 'Um mercado de rua em Chiang Mai virou ponto turístico depois que um blog gastronômico elogiou seus ovos de pato em conserva. O comerciante Khun Somchai agora precisa de fornecimento internacional estável para acompanhar a demanda repentina, já que a produção local tailandesa não supre o volume. Ele paga em dólares e negocia rápido, mas cancela contratos ao menor sinal de atraso.', baseMarket: 12, pricePerUnit: 24, weeklyGoal: 5, durationDays: 90, minLevel: 3, completionBonus: 290, completionXP: 40 },
    { catalogId: 'exp_it', client: '🇮🇹 Confeitaria Bianchi (Modena)', product: 'mel' as const, description: 'Uma pequena confeitaria em Modena, especializada em doces tradicionais com mel, busca fornecedores fora da União Europeia depois que uma doença enfraqueceu as colmeias em toda a região da Emília-Romanha. O confeiteiro Signor Bianchi é obcecado com pureza e aroma floral — encomenda amostras antes de fechar qualquer lote grande, mas quando aprova, vira cliente fiel por anos.', baseMarket: 80, pricePerUnit: 168, weeklyGoal: 3, durationDays: 90, minLevel: 4, completionBonus: 330, completionXP: 45 },
    { catalogId: 'exp_gr', client: '🇬🇷 Cooperativa Papadopoulos (Creta)', product: 'sheep_milk' as const, description: 'Nas montanhas de Creta, uma cooperativa de produtores de queijo feta enfrenta escassez de leite de ovelha depois que o preço da terra expulsou pastores tradicionais para fora da ilha. A cooperativa, liderada pela família Papadopoulos, busca parceiros internacionais confiáveis para não comprometer a receita que atravessa gerações. Exigem análise de gordura e proteína antes de cada embarque.', baseMarket: 28, pricePerUnit: 55, weeklyGoal: 5, durationDays: 120, minLevel: 5, completionBonus: 370, completionXP: 50 },
    { catalogId: 'exp_nl', client: '🇳🇱 Estufas de Aalsmeer', product: 'humus' as const, description: 'Uma rede de estufas de flores em Aalsmeer, próxima ao maior mercado floral do mundo, decidiu migrar para adubação orgânica depois de pressão de consumidores europeus por certificação sustentável. O engenheiro agrônomo Pieter van Dijk testa fornecedores do mundo todo em busca de húmus de minhoca de alta qualidade nutritiva — e paga um prêmio considerável para quem entrega consistência.', baseMarket: 22, pricePerUnit: 44, weeklyGoal: 6, durationDays: 120, minLevel: 6, completionBonus: 410, completionXP: 55 },
    { catalogId: 'exp_kr', client: '🇰🇷 Snail Glow Labs (Seul)', product: 'muco' as const, description: 'A indústria de cosméticos coreana, referência mundial em k-beauty, está sempre à caça de ingredientes exóticos para suas próximas fórmulas "virais". Uma startup de Seul, a Snail Glow Labs, aposta em muco de caracol de fazendas fora da Ásia para diferenciar sua nova linha de séruns anti-idade. A fundadora, Ji-eun, é jovem, ousada e paga adiantado — mas espera inovação e pureza do produto.', baseMarket: 48, pricePerUnit: 103, weeklyGoal: 3, durationDays: 120, minLevel: 7, completionBonus: 450, completionXP: 60 },
    { catalogId: 'exp_be', client: '🇧🇪 Ateliê Margaux Verstappen (Antuérpia)', product: 'angora_wool' as const, description: 'Uma ateliê de moda em Antuérpia, cidade conhecida por seus estilistas vanguardistas, busca lã angorá de procedência ética para uma coleção cápsula sustentável. A estilista Margaux Verstappen visitou fazendas europeias e ficou insatisfeita com o tratamento dos animais — por isso agora prioriza fornecedores que comprovem bem-estar animal, como o seu.', baseMarket: 90, pricePerUnit: 198, weeklyGoal: 2, durationDays: 120, minLevel: 8, completionBonus: 490, completionXP: 65 },
    { catalogId: 'exp_gb', client: '🇬🇧 Alfaiataria Ashworth (Savile Row)', product: 'mohair' as const, description: 'Uma grife inglesa tradicional, com mais de um século de história em Savile Row, Londres, está retomando o uso de mohair fino em seus casacos de inverno feitos sob medida para uma clientela exigente. O alfaiate-chefe, Mr. Ashworth, é rígido quanto à textura e ao brilho da fibra — qualquer lote fora do padrão é recusado sem hesitação, mas os que passam garantem contratos de longo prazo.', baseMarket: 120, pricePerUnit: 252, weeklyGoal: 2, durationDays: 120, minLevel: 9, completionBonus: 530, completionXP: 70 },
    { catalogId: 'exp_in', client: '🇮🇳 Tecelões de Varanasi', product: 'seda_bruta' as const, description: 'Uma cooperativa de tecelões em Varanasi, cidade sagrada às margens do Ganges e capital histórica do sári de seda indiano, enfrenta escassez de matéria-prima depois que doenças atingiram bicheiros locais. O mestre tecelão Ravi Sharma busca seda bruta de qualidade em outros continentes para manter viva uma tradição têxtil de séculos — e paga acima do mercado para não perder prazos religiosos e festivais.', baseMarket: 100, pricePerUnit: 220, weeklyGoal: 2, durationDays: 150, minLevel: 10, completionBonus: 570, completionXP: 75 },
    { catalogId: 'exp_es', client: '🇪🇸 Tapas Bar Don Rafael (Sevilha)', product: 'conserva_codorna' as const, description: 'Uma tapas bar renomada em Sevilha, conhecida por reinventar pratos tradicionais andaluzes, adicionou conserva de codorna ao cardápio depois de um chef visitar a América do Sul e se apaixonar pelo produto. O proprietário, Don Rafael, quer fechar fornecimento fixo antes que o prato vire tendência nacional — e sabe que, se isso acontecer, a demanda vai explodir.', baseMarket: 160, pricePerUnit: 344, weeklyGoal: 2, durationDays: 150, minLevel: 11, completionBonus: 610, completionXP: 80 },
    { catalogId: 'exp_no', client: '🇳🇴 Restaurante Lars Eriksen (Bergen)', product: 'peixe_defumado' as const, description: 'Um restaurante de frutos do mar em Bergen, cidade portuária famosa por seu mercado de peixes centenário, busca fornecedores fora do Atlântico Norte para diversificar o cardápio de defumados durante os meses de escassez local. O chef Lars Eriksen aprecia técnicas diferentes de cura e está disposto a pagar bem por um produto com identidade própria, desde que a cadeia de frio seja impecável.', baseMarket: 70, pricePerUnit: 158, weeklyGoal: 3, durationDays: 150, minLevel: 12, completionBonus: 650, completionXP: 83 },
    { catalogId: 'exp_hu', client: '🇭🇺 Confeitaria Dona Ilona (Budapeste)', product: 'goose_egg' as const, description: 'Uma confeitaria tradicional em Budapeste, famosa por seus doces à base de foie gras e ovos de ganso, perdeu fornecedores locais depois de restrições sanitárias na criação intensiva de aves na região. A proprietária, Dona Ilona, herdou a receita da avó e não abre mão da qualidade do ovo — testa cada lote pessoalmente antes de aprovar o fornecedor definitivo.', baseMarket: 50, pricePerUnit: 115, weeklyGoal: 2, durationDays: 150, minLevel: 13, completionBonus: 680, completionXP: 86 },
    { catalogId: 'exp_ca', client: '🇨🇦 Boutique Amélie Tremblay (Quebec)', product: 'cachecol_angora' as const, description: 'Uma boutique de inverno em Quebec, cidade conhecida por invernos rigorosos e forte cultura de moda artesanal, procura acessórios de lã premium para sua nova coleção "Hiver Chaleureux". A compradora Amélie Tremblay ficou impressionada com a maciez do cachecol de angorá em uma feira internacional e quer garantir exclusividade regional antes que a concorrência descubra o fornecedor.', baseMarket: 260, pricePerUnit: 610, weeklyGoal: 1, durationDays: 150, minLevel: 14, completionBonus: 700, completionXP: 88 },
    // --- Exportação Internacional: segunda leva, 2 opções por nível (1-20), produtos novos sem repetir os 20 acima ---
    { catalogId: 'exp_mx', client: '🇲🇽 Lechería Doña Carmen (Guadalajara)', product: 'milk' as const, description: 'Uma pequena leiteria de bairro em Guadalajara, tradicional há décadas, perdeu parte do fornecimento local depois que uma seca prolongada reduziu os rebanhos da região. Dona Carmen busca leite fresco de procedência confiável para manter a produção diária de suas famosas natas e cremes. Ela paga em dólares e negocia por telefone, mas exige pontualidade religiosa na entrega.', baseMarket: 8, pricePerUnit: 13, weeklyGoal: 3, durationDays: 90, minLevel: 1, completionBonus: 180, completionXP: 27 },
    { catalogId: 'exp_co', client: '🇨🇴 Panadería El Trigal (Medellín)', product: 'egg' as const, description: 'Uma padaria de bairro em Medellín, famosa pelo pandebono e pelas roscas de páscoa, viu a demanda triplicar depois de um vídeo viral nas redes sociais. O padeiro precisa de ovos frescos em volume constante para não decepcionar a fila que se forma toda manhã, e está disposto a pagar acima do mercado por fornecimento garantido.', baseMarket: 4, pricePerUnit: 7, weeklyGoal: 3, durationDays: 90, minLevel: 1, completionBonus: 220, completionXP: 33 },
    { catalogId: 'exp_cl', client: '🇨🇱 Tejeduría Andina (Valparaíso)', product: 'wool' as const, description: 'Uma tecelagem artesanal no porto de Valparaíso, que vende mantas e ponchos para turistas e lojas de decoração, precisa de lã crua de boa qualidade depois que o inverno rigoroso reduziu a tosquia local. A dona do ateliê é exigente com a maciez da fibra, mas fecha contratos de longo prazo com quem entrega direito.', baseMarket: 15, pricePerUnit: 26, weeklyGoal: 2, durationDays: 90, minLevel: 2, completionBonus: 225, completionXP: 32 },
    { catalogId: 'exp_pe', client: '🇵🇪 Quesería Andina (Arequipa)', product: 'cheese' as const, description: 'Uma queijaria familiar em Arequipa, aos pés dos vulcões andinos, abastece mercados de rua e restaurantes turísticos com queijo fresco. Depois que uma epidemia afetou o gado da região, o proprietário foi atrás de fornecedores sul-americanos de confiança — e encontrou na fazenda um parceiro que entrega sem falhar.', baseMarket: 30, pricePerUnit: 56, weeklyGoal: 2, durationDays: 90, minLevel: 2, completionBonus: 275, completionXP: 38 },
    { catalogId: 'exp_ke', client: '🇰🇪 Café Nairobi Gourmet', product: 'butter' as const, description: 'Uma rede de cafeterias em Nairóbi, que serve café especial para expatriados e turistas de safári, quer manteiga artesanal de verdade para seus croissants e pães de fermentação natural. O gerente provou a manteiga da fazenda numa feira gastronômica em Nairóbi e fechou pedido na hora, impressionado com o sabor amanteigado incomum para o padrão local.', baseMarket: 55, pricePerUnit: 96, weeklyGoal: 2, durationDays: 90, minLevel: 3, completionBonus: 261, completionXP: 36 },
    { catalogId: 'exp_za', client: '🇿🇦 Fazenda de Vinhos Stellenbosch', product: 'yogurt' as const, description: 'Uma vinícola boutique em Stellenbosch, que expandiu para um café rural para turistas de enoturismo, quer servir iogurte natural de alta qualidade no brunch dominical. A proprietária, apaixonada por produtos artesanais, testou o iogurte da fazenda e decidiu que nenhum outro serviria — quer exclusividade para o cardápio da propriedade.', baseMarket: 40, pricePerUnit: 76, weeklyGoal: 2, durationDays: 90, minLevel: 3, completionBonus: 319, completionXP: 44 },
    { catalogId: 'exp_eg', client: '🇪🇬 Laticínios do Cairo', product: 'buffalo_milk' as const, description: 'Uma indústria de laticínios no Cairo, tradicional produtora de queijos à base de búfala, enfrenta escassez depois que uma doença reduziu o rebanho bubalino do Delta do Nilo. O comprador egípcio busca fornecedores fora do continente para manter viva a receita do queijo domiati — e paga um prêmio considerável pela regularidade.', baseMarket: 35, pricePerUnit: 62, weeklyGoal: 3, durationDays: 90, minLevel: 4, completionBonus: 297, completionXP: 41 },
    { catalogId: 'exp_ma', client: '🇲🇦 Riad Gastronômico (Marrakech)', product: 'quail_egg' as const, description: 'Um riad boutique em Marrakech, que serve café da manhã de luxo para turistas europeus, adicionou ovos de codorna em conserva ao menu depois que o chef visitou a América do Sul. Os hóspedes adoraram a novidade — agora o riad quer fornecimento fixo antes que outros hotéis da medina copiem a ideia.', baseMarket: 18, pricePerUnit: 35, weeklyGoal: 3, durationDays: 90, minLevel: 4, completionBonus: 363, completionXP: 50 },
    { catalogId: 'exp_tr', client: '🇹🇷 Ateliê de Tapetes de Cappadócia', product: 'alpaca_wool' as const, description: 'Um ateliê de tapetes em Cappadócia, famoso por técnicas de tecelagem milenares, busca fibras nobres alternativas depois que o preço da lã local disparou. A tecelã-chefe ficou impressionada com a maciez da lã de alpaca numa feira em Istambul e quer incorporá-la a uma linha limitada de tapetes de luxo.', baseMarket: 75, pricePerUnit: 137, weeklyGoal: 2, durationDays: 120, minLevel: 5, completionBonus: 333, completionXP: 45 },
    { catalogId: 'exp_pl', client: '🇵🇱 Tricoteria de Cracóvia', product: 'llama_wool' as const, description: 'Uma tricoteria artesanal em Cracóvia, que vende cachecóis e luvas em mercados de Natal europeus, precisa de fibras exóticas para diferenciar sua coleção de inverno. A designer polonesa descobriu a lã de lhama numa revista de moda sustentável e decidiu que era exatamente o toque premium que faltava na vitrine.', baseMarket: 28, pricePerUnit: 55, weeklyGoal: 3, durationDays: 120, minLevel: 5, completionBonus: 407, completionXP: 55 },
    { catalogId: 'exp_ru', client: '🇷🇺 Confeitaria Romanov (São Petersburgo)', product: 'mel_envasado' as const, description: 'Uma confeitaria tradicional de São Petersburgo, que reproduz doces da corte czarista para turistas e clientes nostálgicos, perdeu fornecedores locais depois que o inverno rigoroso dizimou colmeias na região. O confeiteiro exige mel de pureza certificada — testa cada lote antes de aprovar, mas paga muito bem quando aprova.', baseMarket: 320, pricePerUnit: 595, weeklyGoal: 1, durationDays: 120, minLevel: 6, completionBonus: 369, completionXP: 50 },
    { catalogId: 'exp_se', client: '🇸🇪 Queijaria Nórdica (Estocolmo)', product: 'queijo_cabra' as const, description: 'Uma queijaria artesanal em Estocolmo, especializada em queijos de cabra maturados ao estilo escandinavo, enfrenta escassez de matéria-prima depois que o inverno reduziu a produção leiteira local. O mestre queijeiro sueco busca fornecedores do hemisfério sul para manter a produção o ano todo — e não se importa em pagar o frete internacional.', baseMarket: 90, pricePerUnit: 182, weeklyGoal: 2, durationDays: 120, minLevel: 6, completionBonus: 451, completionXP: 61 },
    { catalogId: 'exp_fi', client: '🇫🇮 Café Sauna & Cream (Helsinque)', product: 'iogurte_cabra' as const, description: 'Um café-conceito em Helsinque, que combina sauna urbana com brunch orgânico, quer iogurte de cabra de alta qualidade para sua linha de sobremesas "detox". A proprietária finlandesa é rigorosa sobre certificação de bem-estar animal e testou vários fornecedores antes de aprovar o produto da fazenda.', baseMarket: 70, pricePerUnit: 133, weeklyGoal: 2, durationDays: 120, minLevel: 7, completionBonus: 405, completionXP: 54 },
    { catalogId: 'exp_dk', client: '🇩🇰 Mercado de Queijos de Copenhague', product: 'queijo_pecorino' as const, description: 'Um mercado gourmet em Copenhague, referência em produtos importados de qualidade para a classe alta dinamarquesa, busca queijo pecorino autêntico depois que a demanda por produtos mediterrâneos disparou entre os clientes locais. O comprador dinamarquês é exigente com a textura e o ponto de sal, mas fecha contratos de longo prazo sem hesitar quando aprova o lote.', baseMarket: 280, pricePerUnit: 577, weeklyGoal: 1, durationDays: 120, minLevel: 7, completionBonus: 495, completionXP: 66 },
    { catalogId: 'exp_ch', client: '🇨🇭 Chalé Gourmet (Zermatt)', product: 'iogurte_ovelha' as const, description: 'Um chalé de montanha em Zermatt, que atende esquiadores em busca de café da manhã farto antes das pistas, quer iogurte de ovelha cremoso para acompanhar geleias artesanais da própria estalagem. O proprietário suíço, obcecado por qualidade, provou o produto numa feira e decidiu garantir exclusividade regional.', baseMarket: 80, pricePerUnit: 155, weeklyGoal: 2, durationDays: 120, minLevel: 8, completionBonus: 441, completionXP: 59 },
    { catalogId: 'exp_at', client: '🇦🇹 Confeitaria Habsburgo (Viena)', product: 'ricota_ovelha' as const, description: 'Uma confeitaria centenária em Viena, que serve tortas e strudels para turistas e a alta sociedade local, quer ricota de ovelha fresca para uma nova linha de recheios leves. O confeiteiro austríaco, herdeiro de uma receita imperial, testa cada lote com rigor quase científico antes de aprovar o fornecedor.', baseMarket: 95, pricePerUnit: 200, weeklyGoal: 2, durationDays: 120, minLevel: 8, completionBonus: 539, completionXP: 72 },
    { catalogId: 'exp_ie', client: '🇮🇪 Doceria de Dublin', product: 'doce_leite_ovelha' as const, description: 'Uma doceria artesanal em Dublin, que reinventa sobremesas irlandesas tradicionais com toques latino-americanos, descobriu o doce de leite de ovelha numa viagem gastronômica ao Brasil. A proprietária irlandesa aposta que o produto vai bombar entre os clientes que buscam novidades — e quer ser a primeira a oferecê-lo na cidade.', baseMarket: 160, pricePerUnit: 317, weeklyGoal: 2, durationDays: 120, minLevel: 9, completionBonus: 477, completionXP: 63 },
    { catalogId: 'exp_nz', client: '🇳🇿 Loja de Artesanato de Queenstown', product: 'tapete_lhama' as const, description: 'Uma loja de artesanato em Queenstown, ponto turístico cercado de montanhas na Nova Zelândia, quer tapetes exóticos para complementar sua linha de lã de ovelha local. A proprietária ficou fascinada com o tapete de lhama numa feira de artesanato sul-americano e acredita que vai ser sucesso entre os turistas em busca de peças únicas.', baseMarket: 120, pricePerUnit: 257, weeklyGoal: 1, durationDays: 120, minLevel: 9, completionBonus: 583, completionXP: 77 },
    { catalogId: 'exp_au', client: '🇦🇺 Doceria Colonial (Sydney)', product: 'leite_condensado' as const, description: 'Uma doceria de bairro em Sydney, especializada em sobremesas coloniais para a comunidade de imigrantes latino-americanos, precisa de leite condensado de verdade — sem o gosto industrial das versões locais. A proprietária australiana testou o produto da fazenda e decidiu que era exatamente a textura que faltava nas suas receitas de brigadeiro gourmet.', baseMarket: 145, pricePerUnit: 293, weeklyGoal: 2, durationDays: 150, minLevel: 10, completionBonus: 513, completionXP: 68 },
    { catalogId: 'exp_sg', client: '🇸🇬 Ateliê de Moda de Singapura', product: 'tecido_alpaca' as const, description: 'Um ateliê de moda de luxo em Singapura, que veste a elite asiática com peças exclusivas, busca tecido de alpaca para uma coleção cápsula de inverno tropical (para viagens ao hemisfério norte). A estilista singapuriana é obcecada por sustentabilidade e exige comprovação de bem-estar animal antes de fechar qualquer contrato.', baseMarket: 320, pricePerUnit: 698, weeklyGoal: 1, durationDays: 150, minLevel: 10, completionBonus: 627, completionXP: 83 },
    { catalogId: 'exp_my', client: '🇲🇾 Casa de Chá de Kuala Lumpur', product: 'fio_seda' as const, description: 'Uma casa de chá tradicional em Kuala Lumpur, que também produz acessórios artesanais para complementar a experiência do chá da tarde, busca fio de seda de qualidade depois que fornecedores asiáticos elevaram os preços por causa de uma escassez regional. A proprietária malaia paga em dólares e fecha contratos rápidos quando o produto agrada.', baseMarket: 280, pricePerUnit: 577, weeklyGoal: 1, durationDays: 150, minLevel: 11, completionBonus: 549, completionXP: 72 },
    { catalogId: 'exp_vn', client: '🇻🇳 Mercado Central de Hanói', product: 'carne_jacare' as const, description: 'Um mercado gourmet em Hanói, que atende chefs em busca de ingredientes exóticos para menus de degustação, ouviu falar da carne de jacaré brasileira através de um chef que visitou a América do Sul. O comprador vietnamita quer testar o produto em pratos autorais antes de decidir se fecha fornecimento permanente — mas já paga premium pela amostra.', baseMarket: 300, pricePerUnit: 666, weeklyGoal: 1, durationDays: 150, minLevel: 11, completionBonus: 671, completionXP: 88 },
    { catalogId: 'exp_ph', client: '🇵🇭 Butique de Couro de Manila', product: 'couro_jacare' as const, description: 'Uma butique de acessórios de couro em Manila, que atende clientes de alto poder aquisitivo nas Filipinas, quer couro de jacaré para uma linha limitada de carteiras e cintos. O proprietário filipino viu o material numa feira internacional de couros exóticos e ficou impressionado com a textura e o acabamento — quer exclusividade regional.', baseMarket: 500, pricePerUnit: 1050, weeklyGoal: 1, durationDays: 150, minLevel: 12, completionBonus: 585, completionXP: 75 },
    { catalogId: 'exp_id', client: '🇮🇩 Warung Gourmet (Bali)', product: 'ovo_caipira' as const, description: 'Um warung requintado em Bali, que serve café da manhã orgânico para turistas ocidentais em busca de experiências autênticas, quer ovos caipira de verdade — casca firme, gema alaranjada, criação a céu aberto. O chef indonésio prova cada lote pessoalmente antes de aprovar, mas quando aprova, vira cliente fiel.', baseMarket: 10, pricePerUnit: 23, weeklyGoal: 3, durationDays: 150, minLevel: 12, completionBonus: 715, completionXP: 91 },
    { catalogId: 'exp_tw', client: '🇹🇼 Confeitaria de Taipei', product: 'bolo_caipira' as const, description: 'Uma confeitaria em Taipei, especializada em sobremesas ocidentais para uma clientela jovem e antenada, descobriu o bolo caipira brasileiro através de uma rede social de gastronomia. A proprietária taiwanesa quer ser a primeira a oferecer a novidade na cidade — e está disposta a pagar um preço alto para garantir exclusividade antes da concorrência.', baseMarket: 50, pricePerUnit: 107, weeklyGoal: 2, durationDays: 150, minLevel: 13, completionBonus: 612, completionXP: 77 },
    { catalogId: 'exp_hk', client: '🇭🇰 Boutique de Queijos de Hong Kong', product: 'queijo_minas_jersey' as const, description: 'Uma boutique de queijos finos em Hong Kong, que atende a elite financeira da cidade com produtos importados de procedência rara, quer queijo minas feito com leite Jersey depois que um sommelier de queijos recomendou o produto numa degustação em Xangai. O comprador de Hong Kong só fecha contrato depois de testar a maturação de cada lote.', baseMarket: 95, pricePerUnit: 219, weeklyGoal: 1, durationDays: 150, minLevel: 13, completionBonus: 748, completionXP: 95 },
    { catalogId: 'exp_il', client: '🇮🇱 Laticínios de Tel Aviv', product: 'leite_jersey' as const, description: 'Uma indústria de laticínios boutique em Tel Aviv, que produz queijos e cremes kosher para uma clientela exigente, busca leite Jersey de alta qualidade depois que um relatório sanitário reduziu temporariamente as importações europeias. O comprador israelense exige certificação rigorosa, mas paga bem acima do mercado por regularidade.', baseMarket: 14, pricePerUnit: 31, weeklyGoal: 2, durationDays: 150, minLevel: 14, completionBonus: 630, completionXP: 79 },
    { catalogId: 'exp_sa', client: '🇸🇦 Padaria de Riade', product: 'manteiga_jersey' as const, description: 'Uma padaria de luxo em Riade, que atende a realeza e famílias abastadas com pães e viennoiseries importados, quer manteiga Jersey de altíssima qualidade para suas massas folhadas. O chef francês contratado pela padaria testou vários fornecedores sul-americanos e escolheu o produto da fazenda pela cor e pelo teor de gordura.', baseMarket: 60, pricePerUnit: 140, weeklyGoal: 2, durationDays: 150, minLevel: 14, completionBonus: 770, completionXP: 97 },
    { catalogId: 'exp_qa', client: '🇶🇦 Doceria de Doha', product: 'doce_leite_jersey' as const, description: 'Uma doceria de alto padrão em Doha, que serve sobremesas internacionais para hóspedes de hotéis cinco estrelas, quer doce de leite feito com leite Jersey depois que um chef pâtissier provou o produto numa feira gastronômica em Dubai. O comprador catariano não regateia preço, só exige que a textura seja sempre idêntica de lote a lote.', baseMarket: 120, pricePerUnit: 266, weeklyGoal: 1, durationDays: 165, minLevel: 15, completionBonus: 630, completionXP: 81 },
    { catalogId: 'exp_om', client: '🇴🇲 Café Especial de Mascate', product: 'iogurte_bufala' as const, description: 'Um café especial em Mascate, que atende turistas e expatriados em busca de brunch ocidental de qualidade, quer iogurte de búfala cremoso para suas taças de granola e frutas. O proprietário omanense testou o produto e ficou impressionado com a cremosidade — muito além de qualquer iogurte comum encontrado na região.', baseMarket: 90, pricePerUnit: 214, weeklyGoal: 1, durationDays: 165, minLevel: 15, completionBonus: 770, completionXP: 99 },
    { catalogId: 'exp_cz', client: '🇨🇿 Padaria de Praga', product: 'manteiga_bufala' as const, description: 'Uma padaria histórica em Praga, que assa pães tradicionais tchecos há gerações, quer manteiga de búfala de altíssima qualidade para uma linha especial de brioches destinada a hotéis de luxo. O padeiro tcheco é obcecado pela cor amarelada intensa do produto — diz que nenhuma manteiga europeia chega perto.', baseMarket: 110, pricePerUnit: 249, weeklyGoal: 1, durationDays: 165, minLevel: 16, completionBonus: 675, completionXP: 90 },
    { catalogId: 'exp_ro', client: '🇷🇴 Doceria de Bucareste', product: 'doce_leite_bufala' as const, description: 'Uma doceria boutique em Bucareste, que reinventa sobremesas romenas tradicionais com influências sul-americanas, descobriu o doce de leite de búfala através de um chef que estudou gastronomia no Brasil. A proprietária romena quer ser a única fornecedora da capital antes que a moda se espalhe para outras cidades europeias.', baseMarket: 250, pricePerUnit: 605, weeklyGoal: 1, durationDays: 165, minLevel: 16, completionBonus: 825, completionXP: 110 },
    { catalogId: 'exp_bg', client: '🇧🇬 Cantina Italiana (Sófia)', product: 'burrata' as const, description: 'Uma cantina italiana autêntica em Sófia, que atende uma comunidade crescente de expatriados europeus, perdeu seu fornecedor de burrata depois que uma crise no setor leiteiro italiano encareceu as importações. O chef búlgaro-italiano exige que a burrata chegue cremosa e intacta — testa a textura antes de cada remessa entrar no cardápio.', baseMarket: 380, pricePerUnit: 874, weeklyGoal: 1, durationDays: 165, minLevel: 17, completionBonus: 810, completionXP: 108 },
    { catalogId: 'exp_hr', client: '🇭🇷 Restaurante de Dubrovnik', product: 'pate_pato' as const, description: 'Um restaurante à beira-mar em Dubrovnik, que serve pratos franceses para turistas de cruzeiro em busca de sofisticação, quer patê de pato de alta qualidade para sua entrada mais pedida. O chef croata provou o produto numa feira gastronômica em Zagreb e decidiu que era hora de trocar de fornecedor definitivamente.', baseMarket: 210, pricePerUnit: 517, weeklyGoal: 1, durationDays: 165, minLevel: 17, completionBonus: 990, completionXP: 132 },
    { catalogId: 'exp_rs', client: '🇷🇸 Confeitaria de Belgrado', product: 'ovo_defumado' as const, description: 'Uma confeitaria de vanguarda em Belgrado, que aposta em ingredientes incomuns para pratos de brunch autoral, descobriu o ovo defumado numa viagem gastronômica pela América do Sul. A chef sérvia quer usar o produto como carro-chefe de uma nova entrada — e está disposta a pagar um preço alto para ser a primeira da cidade a oferecê-lo.', baseMarket: 120, pricePerUnit: 281, weeklyGoal: 1, durationDays: 165, minLevel: 18, completionBonus: 900, completionXP: 126 },
    { catalogId: 'exp_is', client: '🇮🇸 Café Rural de Reykjavik', product: 'pudim_caipira' as const, description: 'Um café rural em Reykjavik, que celebra ingredientes de fazenda para turistas em busca de experiências autênticas na Islândia, quer pudim caipira de verdade — receita da vovó, ovos premium, sem atalhos industriais. A proprietária islandesa provou o doce numa feira de gastronomia sustentável e não parou de comentar sobre a textura.', baseMarket: 35, pricePerUnit: 88, weeklyGoal: 2, durationDays: 165, minLevel: 18, completionBonus: 1100, completionXP: 154 },
    { catalogId: 'exp_lu', client: '🇱🇺 Buffet de Eventos de Luxemburgo', product: 'bolinho_peixe' as const, description: 'Um buffet de eventos corporativos em Luxemburgo, que atende reuniões de instituições financeiras internacionais, quer bolinho de peixe brasileiro depois que um executivo provou o petisco numa viagem de negócios ao Brasil. O organizador luxemburguês fechou pedido de amostra grande, prometendo escalar se a qualidade se mantiver constante.', baseMarket: 85, pricePerUnit: 202, weeklyGoal: 1, durationDays: 165, minLevel: 19, completionBonus: 855, completionXP: 117 },
    { catalogId: 'exp_mc', client: '🇲🇨 Restaurante de Mônaco', product: 'moqueca' as const, description: 'Um restaurante de alta gastronomia em Mônaco, que atende uma clientela endinheirada em busca de pratos exóticos e autênticos, quer moqueca brasileira feita com tilápia fresca e leite da própria fazenda. O chef monegasco viajou até a América do Sul só para provar o prato original — e voltou decidido a colocá-lo no cardápio de degustação.', baseMarket: 160, pricePerUnit: 406, weeklyGoal: 1, durationDays: 165, minLevel: 19, completionBonus: 1045, completionXP: 143 },
    { catalogId: 'exp_li', client: '🇱🇮 Mercearia Gourmet de Vaduz', product: 'conserva_peixe' as const, description: 'Uma mercearia gourmet em Vaduz, capital de Liechtenstein, que abastece um público extremamente seleto de residentes e bancos privados, quer conserva de peixe de procedência rara para sua prateleira de importados exclusivos. O comprador liechtensteiniano paga em francos suíços e não questiona preço, só exige regularidade absoluta na entrega.', baseMarket: 140, pricePerUnit: 339, weeklyGoal: 1, durationDays: 210, minLevel: 20, completionBonus: 3600, completionXP: 270 },
    { catalogId: 'exp_va', client: '🇻🇦 Fornecedor Papal (Vaticano)', product: 'manta_premium' as const, description: 'Um fornecedor oficial de eventos do Vaticano, responsável por banquetes diplomáticos e recepções de estado, busca mantas premium de fibras nobres brasileiras para presentear delegações estrangeiras em visita à Santa Sé. O responsável pelas compras é rigoroso com o acabamento e a procedência de cada peça — mas paga uma fortuna pela exclusividade e pelo prestígio de fornecer à Cúria Romana.', baseMarket: 1100, pricePerUnit: 2838, weeklyGoal: 1, durationDays: 210, minLevel: 20, completionBonus: 4400, completionXP: 330 },
  ] as const;
  ...
  const advanceDay = (event: React.MouseEvent) => {
    try {
      ...
      // Incluir o dia atual na contagem de chuva (nextWeather já está calculado acima)
      const effectiveRainyDays = weeklyRainyDays + (nextWeather === 'chuva' ? 1 : 0);
      const rainDiscount = effectiveRainyDays >= 3 ? 0.30 : effectiveRainyDays >= 2 ? 0.20 : effectiveRainyDays >= 1 ? 0.10 : 0;
      ...
      if (currentDay % 7 === 0) {
        ...
        if (Math.random() < 0.3) {
          const MARKET_EVENTS = [
            ...
            { title: '📰 Festival Gastronômico da Vila', desc: 'Pratos da cozinha +30% por 3 dias!', items: ['moqueca','bolinho_peixe','bolo_caipira','pudim_caipira','peixe_defumado','pao_rustico','crepe_rustico'], mult: 1.30, daysLeft: 3 },
            { title: '📰 Semana do Queijo Artesanal', desc: 'Linha Jersey e queijos finos +25% por 3 dias!', items: ['gouda_jersey','queijo_minas_jersey','manteiga_jersey','doce_leite_jersey','queijoBrie','queijo_parmesao','queijo_serra'], mult: 1.25, daysLeft: 3 },
            { title: '📰 Alta do Pescado', desc: 'Peixes e derivados +25% por 2 dias!', items: ['peixe','peixe_defumado','conserva_peixe','bolinho_peixe','moqueca'], mult: 1.25, daysLeft: 2 },
            ...
            { title: '📰 Festival de Ovos', desc: 'Todos os ovos +20% por 3 dias!', items: ['egg','duck_egg','goose_egg','quail_egg','fertile_egg','ovo_caipira'], mult: 1.20, daysLeft: 3 },
      ...
      // Rastrear dias de chuva na semana (para desconto de água na próxima cobrança)
      if (nextWeather === 'chuva' && !isWeeklyBillDay) setWeeklyRainyDays(prev => prev + 1);
            ...
            setTimeout(() => {
              setAnimals(prev => prev.map(a => {
                let penalty =
                  ...
                  a.type === 'bufalo' ? 3 : 0;
                // Poço Nv4+ fornece água extra — mitiga penalidade de rã e jacaré
                if (wellLevel >= 4 && (a.type === 'ra' || a.type === 'jacare')) penalty = Math.max(0, penalty - 3);
                return penalty > 0 ? { ...a, happiness: Math.max(0, a.happiness - penalty) } : a;
              }));
            }, 0);
          }
