            ...
            {atelieTab === 'cozinha' && (
              <div className="space-y-2">
                ...
                {[
                  ...
                  { label: 'Manteiga Jersey', emoji: '🧈', req: `🥛 2 L.Jersey (${(inventory as any).leite_jersey ?? 0}/2) • Nv6`, canCraft: farmLevel >= 6 && ((inventory as any).leite_jersey ?? 0) >= 2, reqLevel: 6, onClick: (e: React.MouseEvent) => c.craftManteigaJersey(e) },
                  { label: 'Queijo Minas Jersey', emoji: '🧀', req: `🥛 3 L.Jersey (${(inventory as any).leite_jersey ?? 0}/3) + 🧂 1 Sal (${(inventory as any).sal ?? 0}/1) • Nv7`, canCraft: farmLevel >= 7 && ((inventory as any).leite_jersey ?? 0) >= 3 && ((inventory as any).sal ?? 0) >= 1, reqLevel: 7, onClick: (e: React.MouseEvent) => c.craftQueijoMinasJersey(e) },
                  { label: 'Doce de Leite Jersey', emoji: '🍮', req: `🥛 2 L.Jersey (${(inventory as any).leite_jersey ?? 0}/2) + 🍯 1 Mel (${(inventory as any).mel ?? 0}/1) • Nv8`, canCraft: farmLevel >= 8 && ((inventory as any).leite_jersey ?? 0) >= 2 && ((inventory as any).mel ?? 0) >= 1, reqLevel: 8, onClick: (e: React.MouseEvent) => c.craftDoceLeiteJersey(e) },
                  { label: 'Gouda Jersey (matura 8d)', emoji: '🧀', req: `🥛 4 L.Jersey (${(inventory as any).leite_jersey ?? 0}/4) + 🧂 1 Sal (${(inventory as any).sal ?? 0}/1) • Nv10`, canCraft: farmLevel >= 10 && ((inventory as any).leite_jersey ?? 0) >= 4 && ((inventory as any).sal ?? 0) >= 1, reqLevel: 10, onClick: (e: React.MouseEvent) => c.craftGoudaJersey(e) },
                  { label: 'Peixe Defumado', emoji: '🐟', req: `🐟 1 Tilápia (${(inventory as any).peixe ?? 0}/1) + 🧂 1 Sal (${(inventory as any).sal ?? 0}/1) • Nv5`, canCraft: farmLevel >= 5 && ((inventory as any).peixe ?? 0) >= 1 && ((inventory as any).sal ?? 0) >= 1, reqLevel: 5, onClick: (e: React.MouseEvent) => c.craftPeixeDefumado(e) },
                  { label: 'Bolinho de Peixe', emoji: '🥟', req: `🐟 1 Tilápia (${(inventory as any).peixe ?? 0}/1) + 🌾 1 Farinha (${(inventory as any).farinha ?? 0}/1) + 🥚 1 Ovo (${inventory.egg}/1) • Nv6`, canCraft: farmLevel >= 6 && ((inventory as any).peixe ?? 0) >= 1 && ((inventory as any).farinha ?? 0) >= 1 && (inventory.egg ?? 0) >= 1, reqLevel: 6, onClick: (e: React.MouseEvent) => c.craftBolinhoPeixe(e) },
                  { label: 'Moqueca da Fazenda', emoji: '🍲', req: `🐟 2 Tilápias (${(inventory as any).peixe ?? 0}/2) + 🥛 1 Leite (${inventory.milk}/1) + 🧂 1 Sal (${(inventory as any).sal ?? 0}/1) • Nv8`, canCraft: farmLevel >= 8 && ((inventory as any).peixe ?? 0) >= 2 && (inventory.milk ?? 0) >= 1 && ((inventory as any).sal ?? 0) >= 1, reqLevel: 8, onClick: (e: React.MouseEvent) => c.craftMoqueca(e) },
                  { label: 'Conserva de Peixe', emoji: '🐟', req: `🐟 2 Tilápias (${inventory.peixe ?? 0}/2) • Nv4`, canCraft: farmLevel >= 4 && (inventory.peixe ?? 0) >= 2, reqLevel: 4, onClick: (e: React.MouseEvent) => c.craftConservaPeixe(e) },
