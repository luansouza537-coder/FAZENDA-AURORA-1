                  ...
                  {queijosEmMaturacao.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 gap-2">
                      {queijosEmMaturacao.map((item, idx) => {
                        const totalDays = item.tipo === 'coalho' ? 3 : item.tipo === 'mucarela' ? 6 : item.tipo === 'buffalo_mozzarella' ? 5 : item.tipo === 'yogurt' ? 2 : item.tipo === 'butter' ? 1 : item.tipo === 'queijo_cabra' ? 2 : item.tipo === 'iogurte_cabra' ? 1 : item.tipo === 'parmesao' ? 15 : item.tipo === 'serra' ? 20 : 12;
