                  ...
                  {queijosEmMaturacao.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 gap-2">
                      {queijosEmMaturacao.map((item, idx) => {
                        const totalDays = item.tipo === 'coalho' ? 1 : item.tipo === 'mucarela' ? 3 : item.tipo === 'buffalo_mozzarella' ? 2 : item.tipo === 'yogurt' ? 1 : item.tipo === 'butter' ? 1 : item.tipo === 'queijo_cabra' ? 2 : item.tipo === 'iogurte_cabra' ? 1 : item.tipo === 'parmesao' ? 15 : item.tipo === 'serra' ? 20 : 7;
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    {[
                      ...
                      { label: 'Queijo de Cabra', emoji: '🧀', req: `🐐 3 L.Cabra (${inventory.goat_milk ?? 0}/3) • ⌛ 2d • Nv3`, canCraft: farmLevel >= 3 && (inventory.goat_milk ?? 0) >= 3 && queijosEmMaturacao.length < maxPrateleiras, reqLevel: 3, onClick: (e: React.MouseEvent) => c.craftQueijoCabra(e) },
