...
export interface AnimalListRowProps {
  ...
  onSellPirarucu?: (id: number, e: React.MouseEvent) => void;
...
export const AnimalListRow: React.FC<AnimalListRowProps> = ({
  ...
  const valueOfPirarucuRow = animal.type === 'tanque_pirarucu' && calculatePirarucuValue ? calculatePirarucuValue(animal) : 0;
  ...
  const isReady =
    ...
    (animal.type === 'boi_angus' && (animal.weightGain || 0) >= 0.8) ||
    (animal.type === 'frango_corte' && (animal.weightGain || 0) >= 0.8) ||
  ...
  const typeLabel: Record<string, string> = {
    vaca: '🐄', ovelha: '🐑', boi: '🐂', galinha: '🐔', cabra: '🐐', peru: '🦃', cabra_angora: '🐐', cabra_boer: '🐐', tanque_pirarucu: '🐋', frango_corte: '🐓', galinha_caipira: '🐔', tanque_tilapia: '🐟', vaca_jersey: '🐄', boi_angus: '🐂', cavalo: '🐴',
  ...
  return (
    ...
    >
      ...
      {animal.isAdult === false && !['minhoca', 'caracol', 'bicho_seda', 'tanque_tilapia'].includes(animal.type) && <span className="text-[9px] bg-blue-100 border border-blue-300 text-blue-700 font-black px-1.5 py-0.5 rounded-full">🍼 {Math.max(0, (animal.adulthoodDay ?? 0) - currentDay)}d</span>}
      ...
      <div className="flex items-center gap-1 ml-auto">
        ...
        {(animal.type === 'boi' || animal.type === 'boi_angus' || animal.type === 'porco' || animal.type === 'peru' || animal.type === 'frango_corte') && <span className="text-[10px] font-mono text-stone-500">🔥{Math.floor((animal.weightGain||0)*100)}%</span>}
      ...
      <div className="flex gap-1 ml-2 shrink-0">
        ...
        {(animal.type === 'galinha' || animal.type === 'galinha_caipira') && animal.hasProducedToday && (
        ...
        {animal.type === 'tanque_tilapia' && animal.fishReady && onCollectPeixe && (
          <button onClick={e => onCollectPeixe(animal.id, e)} className="text-[9px] font-black px-2 py-1 rounded-lg bg-sky-100 border border-sky-300 text-sky-800 hover:bg-sky-200 cursor-pointer">🎣</button>
        )}
        ...
        {animal.type === 'tanque_pirarucu' && onSellPirarucu && (
          <button onClick={e => onSellPirarucu(animal.id, e)} className="text-[9px] font-black px-2 py-1 rounded-lg bg-cyan-100 border border-cyan-300 text-cyan-800 hover:bg-cyan-200 cursor-pointer">💰{valueOfPirarucuRow}</button>
        )}
...
export const AnimalCard: React.FC<AnimalCardProps> = ({
  ...
  const [pendingSellPirarucu, setPendingSellPirarucu] = useState(false);
  ...
  const valueOfAngus = animal.type === 'boi_angus' && calculateAngusValue ? calculateAngusValue(animal) : 0;
  const valueOfBoer = animal.type === 'cabra_boer' && calculateBoerValue ? calculateBoerValue(animal) : 0;
  const valueOfPirarucu = animal.type === 'tanque_pirarucu' && calculatePirarucuValue ? calculatePirarucuValue(animal) : 0;
    ...
    >
      ...
      <div className="flex items-center justify-between gap-3 mb-4">
        <div className="flex-1">
          ...
          <div className="flex flex-wrap gap-1 mt-1">
            {!animal.isAdult && !['minhoca', 'caracol', 'colmeia_abelhas', 'tanque_tilapia'].includes(animal.type) && (
            ...
            {(animal.stressedDays ?? 0) > 0 && !['minhoca', 'caracol', 'colmeia_abelhas', 'tanque_tilapia'].includes(animal.type) && (
            ...
            {isCritical && !['minhoca','caracol','colmeia_abelhas','tanque_tilapia'].includes(animal.type) && (
          ...
          {/* Filhote badge */}
          {!animal.isAdult && animal.adulthoodDay !== undefined && !['minhoca', 'caracol', 'colmeia_abelhas', 'bicho_seda', 'tanque_tilapia'].includes(animal.type) && (
            <span
          ...
          {animal.age !== undefined && animal.maxAge !== undefined && !['minhoca', 'caracol', 'colmeia_abelhas', 'tanque_tilapia'].includes(animal.type) && (() => {
        ...
        <div className="relative group/avatartooltip">
          <div className={`rounded-full w-14 h-14 flex items-center justify-center text-3xl shadow-[inset_0_-4px_0_#d97706] border-4 border-[#f59e0b] shrink-0 relative ${animal.isBestFriend ? 'bg-pink-100 border-pink-400 shadow-pink-500' : 'bg-[#fde68a]'} cursor-help`}>
            ...
            {animal.type === 'boi_angus' && (
              <AngusOx size={46} />
            )}
            {animal.type === 'cavalo' && (
              <span className="select-none" style={{ display: 'inline-block', transform: 'scaleX(-1)' }}>🐴</span>
            )}
            ...
            {animal.type === 'tanque_tilapia' && (
              <>
                <span className="select-none">🐟</span>
                {animal.fishReady && <span className="absolute -bottom-2 -right-1 text-base animate-bounce select-none">🎣</span>}
              </>
            )}
            ...
            {animal.type === 'frango_corte' && (
              <span className="select-none">{getFrangoEmoji ? getFrangoEmoji(animal.weightGain || 0.05) : '🐓'}</span>
            )}
            {animal.type === 'peru' && (
              <span className="select-none">🦃</span>
            )}
            ...
            {animal.type === 'tanque_pirarucu' && (
              <>
                {/* Pirarucu — corpo alongado, escamas com ponta avermelhada (marca da espécie) */}
                <svg width="42" height="42" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg" className="select-none">
                  {/* Corpo alongado, verde-oliva escuro */}
                  <ellipse cx="22" cy="24" rx="19" ry="8" fill="#4a5d3a"/>
                  {/* Barriga mais clara */}
                  <ellipse cx="22" cy="27" rx="17" ry="4.5" fill="#7d9268"/>
                  {/* Faixa de escamas avermelhadas na parte traseira — característica do pirarucu */}
                  <ellipse cx="33" cy="23" rx="7" ry="6.5" fill="#b5442e" opacity="0.85"/>
                  <ellipse cx="38" cy="24" rx="4" ry="5" fill="#c9542f" opacity="0.9"/>
                  {/* Cauda */}
                  <path d="M44 24 L48 18 L48 30 Z" fill="#c9542f"/>
                  {/* Cabeça achatada */}
                  <ellipse cx="8" cy="24" rx="7" ry="6" fill="#4a5d3a"/>
                  {/* Olho */}
                  <circle cx="6" cy="22" r="1.4" fill="#1c1917"/>
                  <circle cx="6.5" cy="21.5" r="0.4" fill="white"/>
                  {/* Barbatana dorsal */}
                  <path d="M18 17 Q24 12 32 16 Q26 18 18 19 Z" fill="#3a4a2d"/>
                </svg>
              </>
            )}
            {animal.type === 'lhama' && (
          ...
          <div className={`border rounded-[18px] p-3 text-center mb-4 text-xs font-extrabold flex flex-col items-center justify-center gap-1.5 uppercase tracking-wide shadow-sm min-h-[58px] ${
            ...
            {animal.type === 'boi_angus' && (
              <div className="flex flex-col gap-1 w-full uppercase">
                <div className="flex justify-between items-center w-full">
                  ...
                  {(animal.weightGain || 0) >= 0.9 && animal.happiness >= 80
                    ? <span className="text-[9px] bg-stone-800 text-amber-300 border border-amber-400 font-black px-1.5 py-0.5 rounded-full animate-pulse">💎 Marmoreio Premium</span>
                    : renderGrowthBadge(animal.weightGain || 0.05)}
                </div>
                <div className="w-full bg-[#e5e7eb] h-2.5 rounded-full overflow-hidden mt-1 border border-stone-300 relative">
                  <div
            ...
            {animal.type === 'tanque_pirarucu' && (
              <div className="flex flex-col gap-1 w-full uppercase">
                ...
                <div className="w-full bg-[#e5e7eb] h-2.5 rounded-full overflow-hidden mt-1 border border-stone-300 relative">
                  <div
                    className="h-full bg-gradient-to-r from-cyan-500 to-cyan-800 rounded-full transition-all duration-300"
                    style={{ width: `${Math.floor((animal.weightGain || 0.15) * 100)}%` }}
                  />
                </div>
                <div className="text-[10px] text-[#78350f]/80 font-mono text-left border-t border-[#fbbf24]/50 pt-1 mt-1.5 leading-none self-start w-full">
                  Pirarucu · Valor na despesca: 💰 ~{valueOfPirarucu} moedas
                </div>
              </div>
            )}
            {animal.type === 'cavalo' && (
            ...
            {animal.type === 'frango_corte' && (
              <div className="flex flex-col gap-1 w-full uppercase">
                ...
                <div className="text-[10px] text-[#78350f]/80 font-mono text-left border-t border-[#fbbf24]/50 pt-1 mt-1.5 leading-none self-start w-full">
                  Pronto em ~8 dias · Valor na Feira: 💰 ~{valueOfFrango} moedas
                </div>
              </div>
            )}
            {animal.type === 'cabra_angora' && (
      ...
      {!collapsed && <div className="flex gap-2 flex-wrap justify-between mt-auto">
        ...
        {(() => {
          ...
          const feedType = (animal.type === 'vaca' || animal.type === 'boi' || animal.type === 'bufalo') ? 'racaoBovina' : animal.type === 'cavalo' ? 'racaoEquina' : animal.type === 'porco' ? 'racaoSuina' : (animal.type === 'ovelha' || animal.type === 'ovelha_leiteira' || animal.type === 'cabra' || animal.type === 'cabra_angora' || animal.type === 'lhama' || animal.type === 'alpaca') ? 'racaoOvinos' : (animal.type === 'galinha' || animal.type === 'codorna' || animal.type === 'pavao' || animal.type === 'peru') ? 'racaoAves' : (animal.type === 'pato' || animal.type === 'ganso') ? 'racaoAquatica' : animal.type === 'coelho_angora' ? 'racaoCoelho' : animal.type === 'tanque_tilapia' ? 'racaoPeixe' : (animal.type === 'ra' || animal.type === 'avestruz' || animal.type === 'jacare') ? 'racaoCarnivora' : 'racaoBovina';
          const feedQty = (inventory as any)[feedType] ?? 0;
          const label = feedType === 'racaoBovina' ? 'Ração Bovina' : feedType === 'racaoEquina' ? 'Ração Equina' : feedType === 'racaoSuina' ? 'Ração Suína' : feedType === 'racaoOvinos' ? 'Ração de Ovinos' : feedType === 'racaoAves' ? 'Ração de Aves' : feedType === 'racaoAquatica' ? 'Ração Aquática' : feedType === 'racaoCoelho' ? 'Ração de Coelhos' : (feedType as string) === 'racaoPeixe' ? 'Ração de Peixes' : 'Ração Carnívora';
        ...
        {animal.type === 'boi_angus' && onSellAngus && (
          pendingSellAngus ? (
            <div className="flex items-center gap-1.5 bg-stone-100 border-2 border-stone-400 rounded-xl px-2 py-1.5 w-full">
              <span className="text-[9px] font-mono font-black text-stone-800 leading-tight flex-1">Vender {animal.name}?<br/><span className="text-amber-600">~{valueOfAngus}💰</span></span>
              <button type="button" onClick={(e) => { e.preventDefault(); setPendingSellAngus(false); onSellAngus(animal.id, e); }}
                className="text-[10px] font-mono font-black px-2 py-1 rounded-lg border-2 border-b-4 border-green-500 bg-green-100 text-green-800 hover:bg-green-200 cursor-pointer transition-all">✅</button>
              <button type="button" onClick={(e) => { e.preventDefault(); setPendingSellAngus(false); }}
                className="text-[10px] font-mono font-black px-2 py-1 rounded-lg border-2 border-b-4 border-stone-400 bg-stone-100 text-stone-700 hover:bg-stone-200 cursor-pointer transition-all">❌</button>
            </div>
          ) : (
            <button
        ...
        {animal.type === 'tanque_pirarucu' && onSellPirarucu && (
          pendingSellPirarucu ? (
            <div className="flex items-center gap-1.5 bg-cyan-50 border-2 border-cyan-300 rounded-xl px-2 py-1.5 w-full">
              <span className="text-[9px] font-mono font-black text-cyan-800 leading-tight flex-1">Despescar {animal.name}?<br/><span className="text-cyan-600">~{valueOfPirarucu}💰</span></span>
              <button type="button" onClick={(e) => { e.preventDefault(); setPendingSellPirarucu(false); onSellPirarucu(animal.id, e); }}
                className="text-[10px] font-mono font-black px-2 py-1 rounded-lg border-2 border-b-4 border-green-500 bg-green-100 text-green-800 hover:bg-green-200 cursor-pointer transition-all">✅</button>
              <button type="button" onClick={(e) => { e.preventDefault(); setPendingSellPirarucu(false); }}
                className="text-[10px] font-mono font-black px-2 py-1 rounded-lg border-2 border-b-4 border-stone-400 bg-stone-100 text-stone-700 hover:bg-stone-200 cursor-pointer transition-all">❌</button>
            </div>
          ) : (
            <button
              type="button"
              onClick={(e) => { e.preventDefault(); setPendingSellPirarucu(true); }}
              className="bg-cyan-700 hover:bg-cyan-600 border-b-4 border-cyan-900 shadow-md rounded-[16px] px-4 py-2.5 font-display text-xs text-white uppercase tracking-wider font-extrabold cursor-pointer flex items-center justify-center gap-1.5 flex-1 select-none transition-all hover:scale-[1.02]"
              title={`Despescar Pirarucu: peixe premium amazônico. 💰 ~${valueOfPirarucu} (peso + bem-estar elevam o valor)`}
            >
              💰 Vender
            </button>
          )
        ...
        {/* Sell Frango de Corte */}
        {animal.type === 'frango_corte' && onSellFrango && (
          pendingSellFrango ? (
            <div className="flex items-center gap-1.5 bg-red-50 border-2 border-red-300 rounded-xl px-2 py-1.5 w-full">
              <span className="text-[9px] font-mono font-black text-red-700 leading-tight flex-1">Vender {animal.name}?<br/><span className="text-red-500">~{valueOfFrango}💰</span></span>
              <button type="button" onClick={(e) => { e.preventDefault(); setPendingSellFrango(false); onSellFrango(animal.id, e); }}
                className="text-[10px] font-mono font-black px-2 py-1 rounded-lg border-2 border-b-4 border-green-500 bg-green-100 text-green-800 hover:bg-green-200 cursor-pointer transition-all">✅</button>
              <button type="button" onClick={(e) => { e.preventDefault(); setPendingSellFrango(false); }}
                className="text-[10px] font-mono font-black px-2 py-1 rounded-lg border-2 border-b-4 border-stone-400 bg-stone-100 text-stone-700 hover:bg-stone-200 cursor-pointer transition-all">❌</button>
            </div>
          ) : (
            <button
              type="button"
              onClick={(e) => { e.preventDefault(); setPendingSellFrango(true); }}
              className="bg-red-500 hover:bg-[#dc2626] border-b-4 border-[#991b1b] shadow-md rounded-[16px] px-4 py-2.5 font-display text-xs text-white uppercase tracking-wider font-extrabold cursor-pointer flex items-center justify-center gap-1.5 flex-1 select-none transition-all hover:scale-[1.02]"
              title={`Vender Frango: venda imediata na Feira. Retorna moedas baseadas no peso (%): 💰 ~${valueOfFrango}`}
            >
              💰 Vender
            </button>
          )
        ...
        {animal.type === 'pato' && (
          <button
            type="button"
            data-onboarding="collect-product-btn" onClick={(e) => { e.preventDefault(); onCollectDuckEgg(animal.id, e); }}
            disabled={!animal.hasProducedToday}
        ...
        {animal.isAdult !== false && animal.type !== 'boi' && animal.type !== 'boi_angus' && animal.type !== 'frango_corte' && animal.type !== 'tanque_tilapia' && animal.type !== 'porco' && animal.type !== 'avestruz' && animal.type !== 'jacare' && animal.type !== 'cabra' && animal.type !== 'ovelha_leiteira' && animal.type !== 'lhama' && animal.type !== 'bufalo' && animal.type !== 'ganso' && animal.type !== 'bicho_seda' && (() => {
          const age = animal.age ?? 0;
        ...
        {animal.type === 'ovelha_leiteira' && animal.isAdult !== false && (
          <button
            type="button"
            data-onboarding="collect-product-btn" onClick={(e) => { e.preventDefault(); onCollectSheepMilk(animal.id, e); }}
        ...
        {animal.type === 'cabra' && animal.isAdult !== false && (
          <button
            type="button"
            data-onboarding="collect-product-btn" onClick={(e) => { e.preventDefault(); onCollectGoatMilk(animal.id, e); }}
            disabled={!animal.isLactating || !animal.hasProducedToday}
            className={`rounded-[16px] px-4 py-2.5 font-display text-xs text-white uppercase tracking-wider font-extrabold flex-1 cursor-pointer flex items-center justify-center gap-1.5 transition-all select-none ${animal.isLactating && animal.hasProducedToday ? 'bg-[#3b82f6] hover:bg-[#2563eb] border-b-4 border-[#1d4ed8] shadow-md active:translate-y-0.5 hover:scale-[1.02]' : 'bg-stone-300 text-stone-500 border-none cursor-not-allowed opacity-60 shadow-none'}`}
        ...
        {animal.type === 'bufalo' && animal.isAdult !== false && (
          <button
            type="button"
            data-onboarding="collect-product-btn" onClick={(e) => { e.preventDefault(); onCollectBuffaloMilk(animal.id, e); }}
            disabled={!animal.hasProducedToday}
